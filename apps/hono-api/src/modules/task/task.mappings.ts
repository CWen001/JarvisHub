...
function extractInlineImageDataUrlsFromGenerateContent(payload: any): string[] {
	...
	const openAiData = Array.isArray((payload as any)?.data) ? (payload as any).data : [];
	for (const item of openAiData) {
		...
		const b64 = typeof (item as any).b64_json === "string" ? (item as any).b64_json.trim() : "";
		if (b64) {
			urls.add(`data:image/png;base64,${b64.replace(/\s+/g, "")}`);
		}
	}
