...
taskRouter.get("/stream", (c) => {
	...
	return streamSSE(c, async (stream) => {
		...
		addTaskProgressSubscriber(userId, subscriber, { replayLatest: true });
...
taskRouter.get("/logs", async (c) => {
	...
	if (!isAdmin || queryUserId) {
		return c.json(
			{
				error: "Vendor call logs are not user-scoped in the current schema",
				code: "vendor_call_logs_not_user_scoped",
			},
			501,
		);
	}
