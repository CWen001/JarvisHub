...
export function resolveApimartImageUrl(baseUrl: string): string {
	const normalized = normalizeApimartBaseUrl(baseUrl);
	if (/\/crawl$/i.test(normalized)) return normalized;
