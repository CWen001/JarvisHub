...
import {
	...
	fetchRightcodeImageTaskResult,
	fetchSeedanceArkTaskResult,
	fetchTuziTaskResult,
} from "./task.service";
import {
...
function taskKindRequiresHostedAssets(kind: TaskKind): boolean {
	return (
		kind === "text_to_image" ||
		kind === "image_edit" ||
		kind === "text_to_video" ||
...
function isStoredTerminalResultReusable(
	c: AppContext,
	result: TaskResultDto,
): boolean {
	if (result.status === "failed") return true;
	if (result.status !== "succeeded") return false;
	if (!taskKindRequiresHostedAssets(result.kind)) return true;
	if (!result.assets.length) return false;
	return result.assets.every((asset) => {
		if (!isHostedAssetUrl(c, asset.url)) return false;
		const thumbnailUrl =
			typeof asset.thumbnailUrl === "string" ? asset.thumbnailUrl.trim() : "";
		return !thumbnailUrl || isHostedAssetUrl(c, thumbnailUrl);
	});
}
...
export async function fetchTaskResultForPolling(
	...
	try {
		...
		if (storedRow?.result) {
			...
			if (parsed.success) {
				...
				if (isStoredTerminalResultReusable(c, parsed.data)) {
					return {
						ok: true,
						vendor:
	...
	let result: any;
		...
		} else if (dispatch === "seedance-ark") {
			result = await fetchSeedanceArkTaskResult(c, userId, taskId, prompt, {
				taskKind: taskKind ?? null,
			});
	...
	} else if (dispatch === "rightcode") {
		if (resolved.kind !== "image") {
			return {
				ok: false,
				status: 400,
				body: {
					error: "rightcode 仅支持图片任务轮询",
					code: "invalid_task_kind",
				},
			};
		}
		result = await fetchRightcodeImageTaskResult(c, userId, taskId, {
			taskKind: (taskKind as any) ?? null,
			promptFromClient: prompt,
		});
