...
type AddTaskProgressSubscriberOptions = {
...
export function addTaskProgressSubscriber(
	...
	options?: AddTaskProgressSubscriberOptions,
): void {
	...
	if (options?.replayLatest === true) {
		for (const snapshot of getPendingTaskSnapshots(userId)) {
			pushTaskProgressSnapshot(subscriber, snapshot);
		}
	}
}
...
export function emitTaskProgress(
	...
	for (const sub of subscribers) {
		pushTaskProgressSnapshot(sub, payload);
	}
}

function pushTaskProgressSnapshot(
	subscriber: TaskProgressSubscriber,
	event: TaskProgressSnapshotDto,
): void {
	try {
