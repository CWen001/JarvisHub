...
import { OPEN_WORKSPACE_USER_ID } from "../auth/open-workspace-data-scope";
...
const SINGLE_WORKSPACE_OWNER_ID = OPEN_WORKSPACE_USER_ID;
...
function mapAccount(row: DreaminaAccountRow): DreaminaAccountDto {
	return DreaminaAccountSchema.parse({
		id: row.id,
		ownerId: SINGLE_WORKSPACE_OWNER_ID,
		label: row.label,
