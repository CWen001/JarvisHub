...
import { registerPublicPptMasterAssetRoutes } from "../task/agents-tool-bridge.ppt-master-asset-routes";
import {
	runApimartTextTask,
	runApimartImageTask,
...
import {
	handlePublicAgentsChatAbortRunRoute,
	handlePublicAgentsChatActiveRunsRoute,
	handlePublicAgentsChatRoute,
	handlePublicAgentsChatRunEventsRoute,
...
import {
	isExecutableAssetTaskKind,
	listExecutableVendorKeysForAssetTaskKind,
} from "../model-catalog/model-catalog.executable";
...
import type { TaskRequestDto, TaskResultDto } from "../task/task.schemas";
...
const PublicAppErrorSchema = z.object({
	...
	error: z.string(),
	code: z.string(),
	details: z.object({}).passthrough().nullable().optional(),
});
...
export function normalizePublicChatAgentStreamEvent(input: {
	...
	if (input.event === "thinking") {
		...
		return text ? { event: "thinking", data } : null;
	}
	if (input.event === "tool") {
		return {
			event: "tool",
...
async function normalizeTaskAssetBackedVideoRequest(
	...
	if (kind === "text_to_video") {
		throw new AppError("text_to_video 已停用；视频生成统一使用 image_to_video 并要求 referenceImages", {
			status: 400,
			code: "text_to_video_disabled",
			details: { requiredKind: "image_to_video" },
		});
	}
	if (kind !== "image_to_video") return request;
	...
	const mergedReferenceImages = Array.from(
		new Set([
			...
			currentLastFrameUrl,
			firstFrameUrlFromAsset,
			lastFrameUrlFromAsset,
	...
	if (mergedReferenceImages.length) {
		...
		throw new AppError("image_to_video 需要至少一张真实参考图 URL", {
			status: 400,
			code: "video_reference_images_missing",
		});
	}
	delete extras.firstFrameUrl;
	delete extras.lastFrameUrl;
	delete extras.firstFrameAssetId;
	delete extras.lastFrameAssetId;
...
function requirePublicUserId(c: AppContext): string {
...
function hasImageEditSourceInExtras(extras: Record<string, unknown>): boolean {
	const referenceImages = Array.isArray(extras?.referenceImages)
		? extras.referenceImages.filter((url): url is string => typeof url === "string" && url.trim().length > 0)
		: [];
...
function normalizeImageEditRequestKind(rawRequest: TaskRequestDto): TaskRequestDto {
	...
	const extras = isRecord(rawRequest.extras) ? { ...rawRequest.extras } : {};
...
function isPublicChatDebugEnabled(c: AppContext, input: Record<string, unknown> | null | undefined): boolean {
...
function pickPublicChatErrorMessageFromResult(result: unknown): string | null {
	const resultRecord = isRecord(result) ? result : {};
	const raw = readRecordField(resultRecord, "raw");
	const response = readRecordField(raw, "response");
	const responseError = readRecordField(response, "error");
	const candidates = [
		raw.failureReason,
		raw.message,
		raw.error,
...
publicApiRouter.get("/agents/chat/runs/active", handlePublicAgentsChatActiveRunsRoute);
publicApiRouter.get("/agents/chat/runs/:runId/events", handlePublicAgentsChatRunEventsRoute);
publicApiRouter.post("/agents/chat/runs/:runId/abort", handlePublicAgentsChatAbortRunRoute);
publicApiRouter.post("/agents/chat", handlePublicAgentsChat);
...
const PublicVisionOpenApiRoute = createRoute({
	...
	description:
		"便捷图像理解接口：创建 image_to_prompt 任务并直接返回文本（常见用法：根据图片反推可复现的英文提示词）。模型来自统一 model config；支持外部 prompt 透传；图片输入支持 imageUrl 或 imageData（二选一）。失败会显式返回错误，不做 draw 降级。",
	request: {
		body: {
			required: true,
			content: {
				"application/json": {
					...
					example: {
						...
						modelAlias: "your-enabled-vision-model",
						temperature: 0.2,
					},
				},
			},
	...
	responses: {
		...
		401: {
			description: "Model config missing or invalid request",
...
publicApiRouter.openapi(PublicVisionOpenApiRoute, async (c) => {
	...
	const hasExplicitModel =
		typeof request.extras.modelAlias === "string" ||
		typeof request.extras.modelKey === "string";
	let vendorCandidates = input.vendorCandidates;
	if (!hasExplicitModel) {
		const defaultModel = await resolveModelConfigDefaultModel(c, "multimodal");
		if (!defaultModel) {
			throw new AppError("未配置全局默认多模态模型", {
				status: 400,
				code: "default_model_not_configured",
				details: { slot: "multimodal" },
			});
		}
		request.extras.modelKey = defaultModel.modelKey;
		vendorCandidates = input.vendorCandidates?.length
			? input.vendorCandidates
			: [defaultModel.vendorKey];
		setTraceStage(c, "public:model:default_applied", {
			modelSelectionSource: "global_default",
			slot: "multimodal",
...
function extractTaskText(result: unknown): string {
	...
	const raw = readRecordField(resultRecord, "raw");
	const response = readRecordField(raw, "response");
	const resultText = readTrimmedRecordString(resultRecord, "text");
	if (resultText) return resultText;
	const direct = readTrimmedRecordString(raw, "text");
	if (direct) return direct;
	const nested = readTrimmedRecordString(response, "text");
	...
	const firstChoice = Array.isArray(response.choices) && response.choices.length > 0 && isRecord(response.choices[0])
		? response.choices[0]
		: {};
	const firstChoiceMessage = readRecordField(firstChoice, "message");
	const choiceContent = readTrimmedRecordString(firstChoiceMessage, "content");
	if (choiceContent) return choiceContent;
	const geminiCandidatesText = Array.isArray(response.candidates)
		? response.candidates
				.flatMap((candidate) => {
					if (!isRecord(candidate)) return [];
					const content = readRecordField(candidate, "content");
...
function pickAutoVendorsForKind(
	...
	const hasReferenceImages = (() => {
		const refs = Array.isArray(extras?.referenceImages)
			? extras.referenceImages
			: [];
		return refs.some((value) => typeof value === "string" && value.trim());
	})();
	const k = (kind || "").trim();
	if (!isPublicTaskKindSupported(k)) return [];
	const allowAgents = k === "chat" || k === "prompt_refine";
	const preferredOrder =
		k === "text_to_image" || k === "image_edit"
			? ["rightcode", "tuzi", "apimart", "gemini", "openai", "yunwu", "qwen"]
		: k === "chat" && hasReferenceImages
			? ["agents", "bltcy", "apimart", "gemini", "openai", "yunwu"]
			: ["agents", "bltcy", "apimart", "gemini", "openai", "yunwu", "qwen", "anthropic", "veo"];
...
async function listEnabledSystemVendors(c: AppContext): Promise<Set<string>> {
	...
	const agentsEnabled = isAgentsBridgeEnabled(c);
...
async function listEnabledUserVendors(c: AppContext, userId: string): Promise<Set<string>> {
	void c;
	void userId;
...
function resolveCatalogKindForTaskKind(
	taskKind: string | null | undefined,
): "multimodal" | "image" | "video" | null {
	const k = (taskKind || "").trim();
	if (!k) return null;
	if (k === "chat" || k === "prompt_refine" || k === "image_to_prompt") return "multimodal";
...
function isCatalogModelKindCompatibleForTaskKind(
	...
	return taskKind?.trim() === "image_to_prompt" && expectedKind === "multimodal" && kindRaw === "image";
}
...
async function resolvePreferredVendorFromModelCatalog(
	...
	const eligibleVendorKeys = await listEligibleVendorKeysByModelKey(c, taskKind, raw);
...
export async function resolvePublicTaskVendors(
	...
	userId: string,
	inputVendor: unknown,
	request: PublicTaskVendorRequest,
	...
	const enabledUserVendors = await (async () => {
		if (!taskKind) return new Set<string>();
		if (taskKind === "chat" || taskKind === "prompt_refine") return new Set<string>();
	...
	const executableAssetVendorKeys =
		isExecutableAssetTaskKind(taskKind)
		? await listExecutableVendorKeysForAssetTaskKind(c, taskKind)
		: null;
	const filterExecutableAutoVendors = (candidates: string[]): string[] => {
		if (!executableAssetVendorKeys) return candidates;
		return candidates.filter((candidate) => {
			const vendorKey = normalizeDispatchVendor(candidate);
			return !!vendorKey && executableAssetVendorKeys.has(vendorKey);
		});
	};

	const defaultAutoCandidates = modelAliasRaw
		? Array.from(enabledVendors.values()).sort((a, b) => a.localeCompare(b))
		: pickAutoVendorsForKind(taskKind, enabledVendors, extras);
	...
	if (isAutoVendor && !vendorCandidates.length && vendorCandidatesHint.length) {
		throw new AppError("指定的候选厂商均未启用或未配置", {
			status: 400,
			code: "no_enabled_vendor_candidate",
			details: {
				kind: taskKind || null,
				vendorCandidates: vendorCandidatesHint,
				systemEnabledVendors: Array.from(enabledSystemVendors.values()),
				userEnabledVendors:
					enabledUserVendors.size > 0 ? Array.from(enabledUserVendors.values()) : [],
			},
	...
	const exactModelKeyMap =
		!modelAliasRaw && modelKeyRaw
			? await resolveVendorModelKeyMapByExactModelKey(c, taskKind, modelKeyRaw)
	...
	if (modelAliasRaw) {
		const aliasEnabledVendors =
			aliasMap && aliasMap.size
				? Array.from(aliasMap.keys()).filter((vendorKey) =>
						enabledVendors.has(vendorKey),
					)
				: [];
		const aliasExecutableVendors =
			isAutoVendor && executableAssetVendorKeys
				? aliasEnabledVendors.filter((vendorKey) =>
						executableAssetVendorKeys.has(vendorKey),
					)
				: aliasEnabledVendors;
		const aliasExecutableVendorSet = new Set(aliasExecutableVendors);
		const supported =
			aliasExecutableVendors.length > 0
				? [
						...vendorCandidates.filter((candidate) => {
							const v = normalizeDispatchVendor(candidate);
							return !!v && aliasExecutableVendorSet.has(v);
						}),
						...aliasExecutableVendors.filter(
							(vendorKey) =>
								!vendorCandidates.some(
									(candidate) =>
										normalizeDispatchVendor(candidate) === vendorKey,
								),
						),
					]
				: [];
		...
		if (!supported.length) {
			if (
				isAutoVendor &&
				executableAssetVendorKeys &&
				aliasEnabledVendors.length > 0 &&
				aliasExecutableVendors.length === 0
			) {
				const capabilityLabel =
					taskKind === "text_to_video" || taskKind === "image_to_video"
						...
						: "图像";
				throw new AppError(
					`模型别名 ${modelAliasRaw} 已存在，但当前没有可执行的${capabilityLabel}接口映射（model_catalog_mappings）`,
					{
						status: 400,
			...
			throw new AppError(
				"未找到可用的模型别名配置（请在 /model-config为该别名配置并启用模型）",
				{
					status: 400,
	...
	if (!isAutoVendor && !modelAliasRaw && modelKeyRaw) {
		...
		if (supportedVendorsByModelKey.size === 0) {
			throw new AppError(
				`未找到已启用模型 ${modelKeyRaw} 的配置，请在 /model-config 中添加并启用该 modelKey`,
				{
					status: 400,
		...
		if (!explicitVendorKey || !supportedVendorsByModelKey.has(explicitVendorKey)) {
			throw new AppError(
				`厂商 ${vendorRaw || "(empty)"} 未配置已启用模型 ${modelKeyRaw}`,
				{
					status: 400,
					code: "model_not_configured",
					details: {
						...
						modelKey: modelKeyRaw,
						vendorRaw: vendorRaw || null,
						configuredVendors: Array.from(supportedVendorsByModelKey.values()).sort((a, b) =>
							a.localeCompare(b),
						),
					},
				},
		...
		if (executableAssetVendorKeys && !executableAssetVendorKeys.has(explicitVendorKey)) {
			const expectedKind = resolveCatalogKindForTaskKind(taskKind);
			const capabilityLabel = expectedKind === "video" ? "视频" : "图像";
			throw new AppError(
				`模型 ${modelKeyRaw} 已存在，但当前没有可执行的${capabilityLabel}接口映射（model_catalog_mappings）`,
				{
					status: 400,
					code: "mapping_not_configured",
					details: {
	...
	if (isAutoVendor && !modelAliasRaw && modelKeyRaw) {
		const executableSupportedVendorsByModelKey =
			executableAssetVendorKeys && supportedVendorsByModelKey.size > 0
				? new Set(
						Array.from(supportedVendorsByModelKey.values()).filter((vendorKey) =>
							executableAssetVendorKeys.has(vendorKey),
						),
					)
				: supportedVendorsByModelKey;
		const expectedKind = resolveCatalogKindForTaskKind(taskKind);
		if (supportedVendorsByModelKey.size === 0) {
			throw new AppError(
				`未找到已启用模型 ${modelKeyRaw} 的配置，请在 /model-config 中添加并启用该 modelKey`,
		...
		if (
			supportedVendorsByModelKey.size > 0 &&
			executableAssetVendorKeys &&
			executableSupportedVendorsByModelKey.size === 0
		) {
			const capabilityLabel = expectedKind === "video" ? "视频" : "图像";
			throw new AppError(
				`模型 ${modelKeyRaw} 已存在，但当前没有可执行的${capabilityLabel}接口映射（model_catalog_mappings）`,
				{
					...
					details: {
						taskKind: taskKind || null,
						modelKey: modelKeyRaw,
						vendors: Array.from(supportedVendorsByModelKey.values()).sort((a, b) =>
							a.localeCompare(b),
						),
					},
				},
		...
		if (executableSupportedVendorsByModelKey.size) {
			vendorCandidates = vendorCandidates.filter((candidate) => {
				const v = normalizeDispatchVendor(candidate);
				return !!v && executableSupportedVendorsByModelKey.has(v);
			});
		} else if (expectedKind) {
			throw new AppError(
				`当前任务 ${taskKind || "(unknown)"} 不支持模型 ${modelKeyRaw}。请改用 ${expectedKind} 类型模型，或传入正确的 modelKey / modelAlias。`,
				{
					status: 400,
	...
	if (isAutoVendor && vendorCandidates.length > 1) {
		...
		const priorityVendors = resolvePublicVendorPriorityList(c, taskKind || null);
	...
	if (!vendorCandidates.length) {
		if (isAutoVendor && !isPublicTaskKindSupported(taskKind)) {
			return Promise.reject(
				Object.assign(new Error("unsupported task kind"), {
					status: 400,
					code: "unsupported_task_kind",
					details: { kind: taskKind || null },
				}),
			);
		}
		...
		const needsNonAgents = taskKind && taskKind !== "chat" && taskKind !== "prompt_refine";
		const message =
			onlyAgents && needsNonAgents
				? `当前仅启用了 Agents（仅支持 chat/prompt_refine）。要运行 ${taskKind}，请在 /model-config启用并配置 OpenAI/Gemini 等厂商 API Key。`
				: "没有可用的全局厂商配置（请在 /model-config启用并配置 API Key）";
		throw new AppError(
			message,
			{
				status: 400,
...
export async function runPublicTask(
	...
	let request = await normalizeTaskAssetBackedVideoRequest(
		c as AppContext,
		userId,
		normalizeImageEditRequestKind(input.request) as TaskRequestDto,
	);
	const effectiveInputVendor = input?.vendor;
	const effectiveInputVendorCandidates = input?.vendorCandidates;
	const extras = (request?.extras || {}) as Record<string, any>;
	setTraceStage(c, "public:run:begin", {
		taskKind: request?.kind ?? null,
		vendor: typeof effectiveInputVendor === "string" ? effectiveInputVendor : null,
		modelAlias:
			typeof extras?.modelAlias === "string" && extras.modelAlias.trim()
				? extras.modelAlias.trim()
				: null,
	...
	const {
		...
		c,
		userId,
		effectiveInputVendor,
		request,
		effectiveInputVendorCandidates,
	);
	...
	for (const vendorCandidate of vendorCandidates) {
		...
		setTraceStage(c, "public:vendor:attempt", {
			...
			let result: any;
				if (v === "apimart") {
					if (requestForVendor.kind === "image_to_video") {
...
// Unified public task API: supports image/video/chat via open single-workspace API.
				...
				"application/json": {
					...
					example: {
						...
						request: {
							...
							extras: {
								modelAlias: "your-video-model-alias",
								durationSeconds: 10,
			...
			content: {
				"application/json": {
					...
					example: {
						vendor: "auto",
						kind: "text_to_image",
						prompt: "一张电影感海报，中文“JarvisHub”，高细节，干净背景",
...
publicApiRouter.openapi(PublicDrawOpenApiRoute, async (c) => {
	...
	let normalizedRequest = normalizeImageEditRequestKind(request) as TaskRequestDto;
	const effectiveInputVendor = input.vendor;
	const effectiveVendorCandidates = input.vendorCandidates;

	const vendorRaw = (effectiveInputVendor || "auto").trim().toLowerCase();
	let dispatchVendor =
	...
	if (preferAsync) {
		if (vendorRaw === "auto") {
			const resolved = await resolvePublicTaskVendors(
				c,
				userId,
				effectiveInputVendor,
				normalizedRequest,
				effectiveVendorCandidates,
			);
	...
	const { vendor, result } = await runPublicTask(c, userId, {
		vendor: effectiveInputVendor,
		vendorCandidates: effectiveVendorCandidates,
...
const PublicVideoOpenApiRoute = createRoute({
	...
	description:
		"便捷视频接口：创建 image_to_video 任务。视频生成必须通过 extras.referenceImages 提供至少一张真实参考图 URL；provider 与模型只来自统一 model config，不读取旧环境变量固定路由。",
	request: {
		body: {
			required: true,
			content: {
		...
		body: {
			...
			content: {
				"application/json": {
					...
					example: { taskId: "task_01HXYZ...", taskKind: "image_to_video" },
				},
			},
		},
	},
