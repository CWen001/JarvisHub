...
agentsRouter.get("/skill", async (c) => {
	...
	const skill = await getPublicAgentSkill(c);
...
agentsRouter.get("/skills", async (c) => {
	const userId = c.get("userId");
	if (!userId) return c.json({ error: "Unauthorized" }, 401);
	const skills = await listPublicAgentSkills(c);
	return c.json(skills.map((s) => AgentSkillSchema.parse(s)));
});

agentsRouter.get("/runtime-skills", async (c) => {
	const userId = c.get("userId");
	if (!userId) return c.json({ error: "Unauthorized" }, 401);
	const result = await listRuntimeAgentSkills(c);
	return c.json(
		RuntimeAgentSkillsResponseSchema.parse({
			skills: result.skills.map((s) => RuntimeAgentSkillSchema.parse(s)),
...
agentsRouter.get("/project-context", async (c) => {
	...
	const result = await getUserProjectWorkspaceContext(c, userId, {
		projectId,
		refresh,
	});
...
agentsRouter.get("/project-context/verify", async (c) => {
	...
	const projectId = (c.req.query("projectId") || "").trim();
	if (!projectId) return c.json({ error: "projectId is required" }, 400);
	const result = await verifyUserProjectWorkspaceContext(c, userId, { projectId });
...
agentsRouter.put("/project-context/file", async (c) => {
	...
	const result = await updateUserProjectWorkspaceContextFile(c, userId, parsed.data);
	return c.json(ProjectWorkspaceContextSchema.parse(result));
});
...
agentsRouter.get("/project-context/version", async (c) => {
	...
	const result = await getUserProjectWorkspaceContextFileVersion(c, userId, {
		projectId,
		fileName,
		versionId,
	});
...
agentsRouter.put("/project-context/rollback", async (c) => {
	...
	const result = await rollbackUserProjectWorkspaceContextFileVersion(c, userId, parsed.data);
...
agentsRouter.get("/pipeline/runs", async (c) => {
	...
	const limit = Number.isFinite(limitRaw)
		...
		: 50;
	const runs = await listUserAgentPipelineRuns(c, userId, {
		projectId,
		limit,
	});
...
agentsRouter.post("/pipeline/runs", async (c) => {
	...
	const run = await createUserAgentPipelineRun(c, userId, parsed.data);
...
agentsRouter.get("/pipeline/runs/:id", async (c) => {
	...
	const run = await getUserAgentPipelineRunById(c, userId, id);
	return c.json(AgentPipelineRunSchema.parse(run));
});
...
agentsRouter.patch("/pipeline/runs/:id/status", async (c) => {
	...
	const run = await updateUserAgentPipelineRunStatus(c, userId, id, parsed.data);
	return c.json(AgentPipelineRunSchema.parse(run));
});
...
adminAgentsRouter.get("/skills", async (c) => {
	const userId = c.get("userId");
	if (!userId) return c.json({ error: "Unauthorized" }, 401);
	const skills = await listAdminAgentSkills(c);
	return c.json(skills.map((s) => AgentSkillSchema.parse(s)));
});
...
adminAgentsRouter.post("/skills", async (c) => {
	...
	const skill = await upsertAdminAgentSkill(c, parsed.data);
...
adminAgentsRouter.get("/diagnostics", async (c) => {
	...
	const limit = Number.isFinite(limitRaw)
		...
		: 50;
	const result = await getAdminAgentDiagnostics(c, userId, {
		projectId,
		label,
		workflowKey,
		turnVerdict,
...
adminAgentsRouter.get("/project-context", async (c) => {
	...
	if (!projectId) return c.json({ error: "projectId is required" }, 400);
	const refresh = String(c.req.query("refresh") || "").trim().toLowerCase() === "true";
	const result = await getAdminProjectWorkspaceContext(c, userId, {
		projectId,
		refresh,
	});
	return c.json(ProjectWorkspaceContextSchema.parse(result));
...
adminAgentsRouter.put("/global-context/file", async (c) => {
	...
	const result = await updateAdminGlobalWorkspaceContextFile(c, parsed.data);
	return c.json(result);
});
...
adminAgentsRouter.get("/global-context/version", async (c) => {
	...
	const result = await getAdminGlobalWorkspaceContextFileVersion(c, { fileName, versionId });
	return c.json(ProjectWorkspaceContextFileVersionContentSchema.parse(result));
});
...
adminAgentsRouter.put("/global-context/rollback", async (c) => {
	...
	const result = await rollbackAdminGlobalWorkspaceContextFileVersion(c, parsed.data);
	return c.json(result);
});
...
adminAgentsRouter.delete("/skills/:id", async (c) => {
	...
	if (!userId) return c.json({ error: "Unauthorized" }, 401);
	const id = c.req.param("id");
	await deleteAdminAgentSkill(c, id);
