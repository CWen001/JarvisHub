...
const PROJECT_CONTEXT_DIR = path.join(".jarvishub", "context");
const GLOBAL_CONTEXT_DIR = path.join(process.cwd(), ".jarvishub", "context");
const GLOBAL_HISTORY_DIR = path.join(process.cwd(), ".jarvishub", "context-history");
...
export async function updateProjectWorkspaceContextFile(input: {
  ...
  const historyDir = path.join(projectRoot, ".jarvishub", "context-history");
...
export async function getProjectWorkspaceContext(input: {
  ...
  const projectHistoryDir = path.join(projectRoot, ".jarvishub", "context-history");
...
export async function ensureProjectWorkspaceContextFiles(
  ...
  const storyStateMarkdown = renderStoryStateMarkdown({ book });
...
async function resolveBookContextSnapshot(input: {
  ...
  if (!raw.trim()) throw new Error(`book index is empty: ${indexPath}`);
  const parsed = parseJsonRecord(raw);
  if (!parsed) throw new Error(`book index is invalid JSON: ${indexPath}`);
...
async function collectBookIndexCandidates(
  ...
  for (const booksRoot of roots) {
    if (!(await pathExists(booksRoot))) continue;
...
function renderProjectMarkdown(input: {
  ...
  const lines = [
    "# JarvisHub Project Context",
    "",
    "- 这是项目专属上下文；平台共用规则见工作区 `.jarvishub/context/GLOBAL_RULES.md`。",
    `- projectId: ${input.projectId}`,
...
function renderCharactersMarkdown(input: { book: BookContextSnapshot | null }): string {
  ...
  if (!input.book || input.book.characters.length === 0) {
    lines.push("- 当前 book index 未提取到角色卡，请以章节脚本与已生成资产为准。", "");
    return `${lines.join("\n")}\n`;
  }
...
async function loadContextFilesFromDir(input: {
  ...
  const extraEntries = (await pathExists(input.dirPath)) ? await fs.readdir(input.dirPath) : [];
  const allNames = [
    ...explicitOrder,
  ...
  for (const fileName of allNames) {
    ...
    let loaded: ProjectWorkspaceContextFileDto | null = null;
    try {
      loaded = await loadContextFile({
      ...
      if (error instanceof AppError && error.code === "context_file_not_found") continue;
      throw error;
    }
    if (!loaded) continue;
    files.push(loaded);
...
async function loadContextFile(input: {
  ...
  return {
    path: input.layer === "global" ? path.join(".jarvishub", "context", input.fileName) : path.join(PROJECT_CONTEXT_DIR, input.fileName),
    content,
    layer: input.layer,
