...
export type StorageTarget = { kind: "rustfs"; config: RustfsConfig };

export async function uploadToStorageFromUrl(options: {
	c: AppContext;
	userId: string;
	sourceUrl: string;
	prefix?: string;
...
export async function persistGeneratedTaskAssets(options: {
	c: AppContext;
	userId: string;
	...
	return persistHostedTaskAssets({ c, userId, assets: normalized, meta });
}

export function isHostedAssetUrl(c: Pick<AppContext, "env" | "req">, url: string): boolean {
	const trimmed = (url || "").trim();
	if (!trimmed) return false;
	const publicBase = resolvePublicAssetBaseUrl(c as AppContext).trim().replace(/\/+$/, "");
	if (publicBase && trimmed.startsWith(`${publicBase}/`)) return true;
	try {
		const requestUrl = new URL(c.req.url);
		const proxyBase = `${requestUrl.origin}/assets/r2`;
		if (trimmed.startsWith(`${proxyBase}/`)) return true;
	} catch {
		// ignore invalid request URLs
	}
	if (/^\/assets\/r2\/gen\//.test(trimmed)) return true;
	return /^\/?gen\//.test(trimmed);
}

export async function persistHostedTaskAssets(options: {
	c: AppContext;
	userId: string;
	assets: TaskAssetDto[] | undefined;
	meta?: {
	...
	const isHostedUrl = (url: string): boolean => isHostedAssetUrl(c, url);
	...
	for (const asset of assets) {
		...
		try {
			...
			if (existing && existing.data) {
				...
				try {
					existingRowData = JSON.parse(existing.data);
				} catch {
					existingRowData = null;
				}

				const existingUrl =
					existingRowData && typeof existingRowData.url === "string"
						? existingRowData.url.trim()
						: "";
				if (existingUrl && isHostedUrl(existingUrl)) {
					const existingThumb =
						typeof existingRowData.thumbnailUrl === "string"
							? existingRowData.thumbnailUrl
		...
		if (inlineData && inlineBytes) {
			...
			value = TaskAssetSchema.parse({ ...value, url: uploaded.url });
		} else if (!isHostedUrl(originalUrl)) {
			if (hostingDisabled) {
				throw new AppError(
					"Asset hosting is disabled; generated assets must be persisted before returning to canvas",
					{
						status: 500,
						code: "asset_hosting_disabled",
						details: { sourceUrl: stripUrlSearchAndHash(originalUrl) },
					},
				);
			}
			...
			try {
				...
				if (existingRowId) {
					...
					const baseData =
						existingRowData && typeof existingRowData === "object"
							? existingRowData
							: {};
					await updateAssetDataRow(c.env.DB, userId, existingRowId, {
						...baseData,
						hosting: {
							status: "failed" as AssetHostingStatus,
				...
				console.warn(
					"[asset-hosting] upload failed, marked as failed",
		...
		if (thumbRaw && thumbRaw !== value.url && !isHostedUrl(thumbRaw)) {
			...
			if (inlineThumb) {
				...
				if (hostingDisabled) {
					throw new AppError(
						"Asset hosting is disabled; generated thumbnails must be persisted before returning to canvas",
						{
							status: 500,
							code: "asset_hosting_disabled",
							details: { sourceUrl: stripUrlSearchAndHash(thumbRaw) },
						},
					);
				}
		...
		try {
			...
			resultAsset = TaskAssetSchema.parse({
				...resultAsset,
				sourceUrl: lookupSource,
				modelInputUrl: lookupSource,
			});
