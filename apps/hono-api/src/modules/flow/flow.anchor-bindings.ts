...
export const PUBLIC_FLOW_REFERENCE_RELATIONSHIP_KINDS = [
	"primary",
	"reference",
] as const;
...
export type PublicFlowReferenceRelationshipKind =
	(typeof PUBLIC_FLOW_REFERENCE_RELATIONSHIP_KINDS)[number];
...
export type PublicFlowAnchorBinding = {
	...
	note?: string | null;
	weight?: number | null;
	relationshipKind?: PublicFlowReferenceRelationshipKind | null;
};
...
function normalizeReferenceRelationshipKind(
	value: unknown,
): PublicFlowReferenceRelationshipKind | null {
	const normalized = readTrimmedString(value).toLowerCase();
	for (const candidate of PUBLIC_FLOW_REFERENCE_RELATIONSHIP_KINDS) {
		if (candidate === normalized) return candidate;
	}
	return null;
}
...
function buildAnchorBindingKey(binding: PublicFlowAnchorBinding): string {
	return [
		...
		binding.weight == null ? "" : String(binding.weight),
		readTrimmedString(binding.relationshipKind).toLowerCase(),
	].join("\u0001");
}
...
export function normalizePublicFlowAnchorBinding(
	...
	const relationshipKind = normalizeReferenceRelationshipKind(record.relationshipKind);
