...
type PublicFlowTaskNodeCoreType = "text" | "image" | "video" | "web";
...
> = {
	text: {
		targets: new Set<string>(["in-any"]),
		sources: new Set<string>(["out-text", "out-text-wide", "out-pptx"]),
	},
	image: {
	...
	web: {
		targets: new Set<string>(["in-any"]),
		sources: new Set<string>(["out-text"]),
	},
};
