...
import { modelConfigRouter } from "./modules/model-config/model-config.routes";
...
import { publicApiRouter } from "./modules/apiKey/apiKey.routes";
...
import { snapshotRouter } from "./modules/snapshot";
...
import { authRuntimeRouter } from "./modules/auth/auth.runtime.routes";
...
export async function createCanvasApp(): Promise<OpenAPIHono<AppEnv>> {
		...
		cors({
			...
			allowMethods: ["GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"],
				...
				allowHeaders: [
					...
					"X-File-Name",
					"X-File-Size",
					"X-JarvisHub-No-Retry",
					"X-JarvisHub-Source",
					"x-canvas-referrer-path",
					"x-canvas-page-path",
				],
				credentials: true,
			}),
	...
	app.get("/openapi.json", (c) => {
		...
		const mergedTags = (() => {
			...
			for (const tag of [...(rootDoc.tags ?? []), ...(publicDoc.tags ?? [])]) {
				if (!isObjectRecord(tag)) continue;
		...
		const mergedComponents = (() => {
			const a = isObjectRecord(rootDoc.components) ? rootDoc.components : {};
			const b = isObjectRecord(publicDoc.components) ? publicDoc.components : {};
			const mergeRecord = (key: string) => ({
				...(isObjectRecord(a[key]) ? a[key] : {}),
				...(isObjectRecord(b[key]) ? b[key] : {}),
			});
			...
			const hasAny = Object.keys(merged).some(
				(k) => isObjectRecord(merged[k]) && Object.keys(merged[k]).length > 0,
			);
