...
function readAgentsCliCanvasConfig(repoRoot: string): {
	canvasApiKey?: string;
	...
	try {
		...
		const canvasApiKey =
			typeof parsed.canvasApiKey === "string" ? parsed.canvasApiKey.trim() : "";
		const canvasApiBaseUrl =
			typeof parsed.canvasApiBaseUrl === "string"
				? parsed.canvasApiBaseUrl.trim()
				: "";
		return {
...
export async function maybeAutostartAgentsBridge(): Promise<void> {
	...
	const cliTapConfig = readAgentsCliCanvasConfig(repoRoot);
	...
	const childEnv = {
		...
		...(process.env.CANVAS_API_KEY
			? {}
			: cliTapConfig.canvasApiKey
				? {
					CANVAS_API_KEY: cliTapConfig.canvasApiKey,
					canvasApiKey: cliTapConfig.canvasApiKey,
				}
				: {}),
		...(process.env.CANVAS_API_BASE_URL
			? {}
			: cliTapConfig.canvasApiBaseUrl
				? {
					CANVAS_API_BASE_URL: cliTapConfig.canvasApiBaseUrl,
					canvasApiBaseUrl: cliTapConfig.canvasApiBaseUrl,
				}
				: {}),
