...
import { readBrandedEnv, readBrandedEnvBool } from "../utils/brandedEnv";
...
export function resolveDevPublicBypassFromContext(c: AppContext): DevPublicBypassState | null {
	const enabled = readBrandedEnvBool(c.env, "DEV_PUBLIC_BYPASS");
	if (!enabled) return null;

	const secret = readBrandedEnv(c.env, "DEV_PUBLIC_BYPASS_SECRET");
	if (!secret) return null;

	const provided = normalizeHeaderValue(c.req.header("x-jarvishub-dev-bypass"));
	if (!provided || provided !== secret) return null;

	const host = readHostNameFromHostHeader(c.req.header("host") || "");
	if (!isLoopbackHost(host)) return null;

	const userId = readBrandedEnv(c.env, "DEV_PUBLIC_BYPASS_USER_ID") || "dev-local";
	const role = readBrandedEnv(c.env, "DEV_PUBLIC_BYPASS_ROLE") || "admin";
