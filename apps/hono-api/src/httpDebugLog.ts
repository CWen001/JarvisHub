...
function redactInlineImageDataUris(input: string): string {
	const redactedDataUris = input.replace(
	...
	return redactedDataUris.replace(
...
function safeJsonForLog(c: AppContext | null, value: unknown): unknown {
	const unsafe = !c || isHttpDebugLogUnsafeEnabled(c);
	const seen = new WeakSet<object>();
	const walk = (v: any): any => {
		if (typeof v === "string") return redactInlineImageDataUris(v);
		if (!v || typeof v !== "object") return v;
		if (seen.has(v)) return "[Circular]";
		seen.add(v);
		if (Array.isArray(v)) return v.map(walk);
		const isInlineBase64Envelope =
			typeof v.type === "string" && v.type.trim().toLowerCase() === "image_base64";
		const out: Record<string, any> = {};
		for (const [key, val] of Object.entries(v)) {
			if (!unsafe && SENSITIVE_JSON_KEYS.has(key.toLowerCase())) {
				out[key] = "***";
				continue;
			}
			if (isInlineBase64Envelope && key === "value" && typeof val === "string") {
				out[key] = `[inline-base64 len=${val.replace(/\s+/g, "").length}]`;
				continue;
			}
			out[key] = walk(val);
		}
