export const API_DOCS_ZH_MD = `# JarvisHub Hono API 接口文档（可复制）
...
- 登录用户（画布内推荐）：
  - \`Authorization: Bearer <jh_token>\`（登录 JWT；用于鉴权/计费/资源归属）
- 外部系统（推荐）：
  - \`X-API-Key: tc_sk_...\`
  - 或（兼容）\`Authorization: Bearer tc_sk_...\`
- 同时携带（可选）：
  - \`X-API-Key: tc_sk_...\`（通道 Key）
  - \`Authorization: Bearer <jh_token>\`（登录 JWT）
...
- \`extras.modelAlias\` 用于选择模型（Public 统一别名；推荐在需要跨厂商统一入口时显式配置）。不同厂商可以配置同一个别名，从而让外部调用不用关心具体厂商的 modelKey；未配置 alias 的模型不会自动使用 modelKey 作为 alias。
