const apiBase = String(process.env.JARVISHUB_LOCAL_API_BASE || 'http://localhost:8788').trim().replace(/\/+$/, '')
const localAuthToken = String(process.env.JARVISHUB_LOCAL_AUTH_TOKEN || '').trim()
...
const projectId = String(process.env.JARVISHUB_PROJECT_ID || '').trim()
...
if (!localAuthToken) fail('missing JARVISHUB_LOCAL_AUTH_TOKEN')
