...
const OUTPUT_DIR =
  process.env.SCREENSHOT_OUTPUT_DIR || "/tmp/canvas-design-checklist";
...
async function assertChapterProductionControlsAbsent(page) {
  ...
  const matches = forbiddenTexts.filter((text) => bodyText.includes(text));
  if (matches.length > 0) {
    throw new Error(`chapter production controls should be absent: ${matches.join(", ")}`);
  }
}
...
async function main() {
  ...
  await page.addInitScript(
    ({ smokeToken, user }) => {
      window.localStorage.setItem("jh_token", smokeToken);
      window.localStorage.setItem("jh_user", JSON.stringify(user));
      document.cookie = `jh_token=${encodeURIComponent(smokeToken)}; Path=/; SameSite=Lax`;
      window.localStorage.setItem(
        `canvas-project-manager-tour:v2:${String(user.sub)}`,
        "1",
      );
      window.localStorage.setItem("canvas.aiChat.layoutPreference.v1", JSON.stringify({
        dockRight: true,
        mode: "expanded",
      }));
    },
  ...
  try {
    ...
    await closeTourIfNeeded(page);
    await assertChapterProductionControlsAbsent(page);
    ...
    outputs.chapterPage = await capture(page, "chapter-page.png");
