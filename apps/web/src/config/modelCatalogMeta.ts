...
import { normalizeVideoResolution } from '../utils/videoSpec'
...
export type VideoModelControlBinding =
  | 'durationSeconds'
  | 'size'
  | 'resolution'
  | 'orientation'
...
function parseControlBinding(value: unknown): VideoModelControlBinding | null {
  ...
  if (raw === 'generateaudio' || raw === 'generate_audio' || raw === 'audio') {
    return 'generateAudio'
  }
  if (raw === 'returnlastframe' || raw === 'return_last_frame' || raw === 'lastframe') {
...
function defaultControlLabel(binding: VideoModelControlBinding): string {
  ...
  if (binding === 'generateAudio') return '声音'
  if (binding === 'returnLastFrame') return '尾帧'
  return '画幅'
}
...
function defaultControlOptionSource(binding: VideoModelControlBinding): VideoModelControlOptionSource {
  ...
  if (binding === 'generateAudio') return 'generateAudioOptions'
  if (binding === 'returnLastFrame') return 'returnLastFrameOptions'
  return 'sizeOptions'
}
...
function parseControlOptionSource(
  ...
  if (raw === 'generateaudiooptions' || raw === 'generateaudio' || raw === 'generate_audio' || raw === 'audio') {
    return 'generateAudioOptions'
  }
  if (raw === 'returnlastframeoptions' || raw === 'returnlastframe' || raw === 'return_last_frame' || raw === 'lastframe') {
    return 'returnLastFrameOptions'
  }
