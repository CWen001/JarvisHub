...
/**
 * 判断该 kind 是否有真实的远程 runner（即 runNodeRemote 内部对该 kind 有专属分支）。
 * 这是 dispatcher 唯一应使用的判定——不要把 coreType 当作「应走远程」的代理。
 *
 * 当前命中条件：
 *   - kind === 'webHero'：runNodeRemote → runWebHeroNode（remoteRunner.ts）
 *   - coreType === 'image' 或 'video'：runNodeRemote 内的通用图像/视频分支
 *
 * 新 kind 要走远程时：先在 remoteRunner.ts 加分支，再把它列入此函数。
 */
function isRemoteRunnableKind(kind: string | null | undefined): boolean {
  if (typeof kind !== 'string') return false
  const trimmed = kind.trim()
  if (!trimmed) return false
  if (trimmed === 'webHero') return true
  const coreType = getTaskNodeCoreType(trimmed)
    ...
    while (running < concurrency && ready.length) {
      ...
      void (async () => {
        try {
          ...
          if (isRemoteRunnableKind(typeof kind === 'string' ? kind : null)) {
