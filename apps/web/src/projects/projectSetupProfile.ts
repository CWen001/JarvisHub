...
export type ProjectSetupProfile = {
  ...
  version: 1
  projectType: string
  creationMode: 'text-upload' | 'blank-canvas'
  intro: string
  ...
  createdFrom: 'uploaded-text' | 'blank-canvas'
...
function normalizeCreationMode(value: unknown): ProjectSetupProfile['creationMode'] {
  return value === 'blank-canvas' ? 'blank-canvas' : 'text-upload'
}
...
function normalizeCreatedFrom(value: unknown): ProjectSetupProfile['createdFrom'] {
  return value === 'blank-canvas' ? 'blank-canvas' : 'uploaded-text'
}
