...
import { useOrthEdgeObstacles, type OrthEdgeObstacle } from '../CanvasRenderContext'
...
function orthPathAvoid(
  sx: number,
  sy: number,
  tx: number,
  ty: number,
  obstacles: readonly OrthEdgeObstacle[],
) {
  const dir = sx < tx ? 1 : -1
...
export default function OrthTypedEdge(props: EdgeProps<any>) {
  ...
  const obstacles = useOrthEdgeObstacles()
