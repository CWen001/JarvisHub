...
function formatSequenceBadge(value: number | null | undefined): string {
...
export function ImageContent(props: ImageContentProps) {
  ...
  const normalizedNodeLabel = String(nodeLabel || '').trim()
  const sequenceBadge = formatSequenceBadge(sequenceNo)
  const showIdentityOverlay = !isExpanded && Boolean(normalizedNodeLabel || sequenceBadge)
  ...
  const handleMainImageLoad = React.useCallback(
    (e: React.SyntheticEvent<HTMLImageElement>) => {
      ...
      if (naturalWidth > 0 && naturalHeight > 0 && activeImageUrl && activeImageUrl !== mediaAutoSized) {
        const profile = getNodeSizeProfile({ coreType: 'image' })
        const targetHeight = computeMediaNodeHeight(nodeWidth, naturalWidth, naturalHeight, profile)
        const patch: Record<string, unknown> = {
          mediaAutoSized: activeImageUrl,
          naturalWidth,
          naturalHeight,
        }
    ...
    [activeImageUrl, mediaAutoSized, nodeWidth, nodeHeight, onUpdateNodeData],
  )
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showIdentityOverlay && sequenceBadge && (
          <div
            className="task-node-image__sequence-badge"
            style={{
              position: 'absolute',
              top: 10,
              left: nodeKind === 'imageFission' ? 70 : 10,
              zIndex: 9,
              minWidth: 30,
              height: 24,
              padding: '0 8px',
            ...
            title={`第 ${Math.trunc(sequenceNo || 0)} 张`}
          >
            {sequenceBadge}
          </div>
        )}

        {showIdentityOverlay && normalizedNodeLabel && (
          <div
            className="task-node-image__title-strip"
            style={{
              position: 'absolute',
              left: 0,
              right: 0,
          ...
          >
            <div
              className="task-node-image__title-text"
              style={{
                maxWidth: '100%',
                color: '#fff',
                fontSize: 12,
                ...
                whiteSpace: 'nowrap',
                textShadow: '0 1px 5px rgba(0,0,0,0.38)',
              }}
              title={normalizedNodeLabel}
            >
              {normalizedNodeLabel}
            </div>
          </div>
          ...
          {variantEntries.map(({ result: r, index }) => (
            (() => {
              ...
              <button
                ...
                onDragStart={(evt) => {
                  evt.stopPropagation()
                  setJarvisHubImageDragData(evt, r.url, {
