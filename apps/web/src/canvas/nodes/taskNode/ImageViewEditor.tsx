...
} from '@jarvishub/image-view-controls'

import { toast } from '../../../ui/toast'
import { isRemoteUrl } from './utils'
import { decideImageViewEditMode, type ImageViewEditMode } from './imageViewEditMode'
...
type UseImageViewEditorOptions = {
  ...
  /** Current node kind — used to pick the default applyTarget. */
...
export function useImageViewEditor(options: UseImageViewEditorOptions) {
  ...
  const [applyTarget, setApplyTarget] = React.useState<ImageViewEditMode>(() =>
    decideImageViewEditMode({ kind: options.nodeKind ?? null, preference: null }),
  )
  ...
  const openEditor = React.useCallback((nextMode: ViewEditorMode) => {
    ...
    setApplyTarget(decideImageViewEditMode({ kind: options.nodeKind ?? null, preference: null }))
    setMode(nextMode)
  }, [options.baseImageUrl, options.cameraControl, options.hasImages, options.lightingRig, options.nodeKind])
  ...
  const handleApply = React.useCallback(() => {
    ...
    setMode(null)
  }, [applyTarget, cameraControl, lightingRig, mode, options])
      ...
      <div className="tc-image-view-editor__shell" style={{ display: 'grid', gridTemplateColumns: 'minmax(360px, 1fr) minmax(320px, 0.9fr)', gap: 16 }}>
        ...
        >
          <Stack className="tc-image-view-editor__controls-stack" gap="md">
            ...
            <Group className="tc-image-view-editor__actions" justify="space-between" gap="xs" wrap="nowrap">
              <SegmentedControl
                className="tc-image-view-editor__apply-target"
                size="xs"
                value={applyTarget}
                onChange={(value) => setApplyTarget(value === 'inPlace' ? 'inPlace' : 'newNode')}
                data={[
                  { value: 'newNode', label: '生成新节点' },
                  { value: 'inPlace', label: '原地替换' },
                ]}
              />
              <Group gap="xs">
                <Button
