...
export type TaskNodeKind = 'text' | 'video' | 'image' | 'imageEdit' | 'character' | 'webHero' | 'pptDeck'

export type TaskNodeCoreType = 'text' | 'video' | 'image'
...
const TASK_NODE_SCHEMAS: Record<TaskNodeKind, TaskNodeSchema> = {
  ...
  character: {
    ...
    icon: IconTypography,
    label: '角色',
    features: ['prompt', 'character', 'characterMentions'],
    handles: {
      sources: [{ id: 'out-text', type: 'text', position: SOURCE }],
    },
  },
  ...
  pptDeck: {
    ...
    label: 'PPT',
    features: ['prompt', 'pptPreview'],
    handles: {
      targets: [{ id: 'in-any', type: 'any', position: TARGET }],
      sources: [{ id: 'out-pptx', type: 'pptx', position: SOURCE }],
    },
  },
}
...
const TASK_NODE_DEFINITIONS: TaskNodeSchemaDefinition[] = Object.values(TASK_NODE_SCHEMAS).map((schema) => ({
  coreType: schema.kind === 'imageEdit' ? 'image' : schema.kind === 'character' || schema.kind === 'webHero' || schema.kind === 'pptDeck' ? 'text' : schema.kind,
  schema,
}))
