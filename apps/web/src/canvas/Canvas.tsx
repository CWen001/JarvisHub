...
import { IconBoxMultiple, IconBrackets, IconLayoutGridAdd, IconPhoto, IconPlayerPlay, IconTypography, IconVideo, IconWorldWww } from '@tabler/icons-react'
import { getCurrentLanguage, setLanguage, $, $t } from './i18n'
import TypedEdge from './edges/TypedEdge'
import OrthTypedEdge from './edges/OrthTypedEdge'
import { getCanvasEdgeOpacity, readCanvasEdgeRelationship } from './edges/edgeRelationship'
...
import { createServerAsset, listProjectFlows, listProjects, recoverUploadedServerAssetFile, saveProjectFlow, upsertProject, uploadServerAssetFile, type ProjectDto } from '../api/server'
...
import { getJarvisHubImageDragPayload } from './dnd/setJarvisHubImageDragData'
import { pickVideoTailFrameFromNode } from './nodes/taskNode/upstreamReferences'
...
import { CanvasRenderContext, OrthEdgeObstaclesContext, type OrthEdgeObstacle } from './CanvasRenderContext'
...
type CanvasFocusWindow = Window & {
  ...
  __tcLastImagePasteAt?: number
  __tcLastWorkflowPasteAt?: number
}

type FlowNodeWithDragHandle = FlowNode & {
  dragHandle?: unknown
}

type GlobalImageProvider = typeof globalThis & {
...
function readFlowNodeData(node: FlowNode | null | undefined): Record<string, unknown> {
...
function hasFlowNodeDragHandle(node: FlowNode): boolean {
  return typeof (node as FlowNodeWithDragHandle).dragHandle !== 'undefined'
}

function stripFlowNodeDragHandle(node: FlowNode): FlowNode {
  const { dragHandle: _dragHandle, ...rest } = node as FlowNodeWithDragHandle
  return rest
}

function getNodeDataString(node: FlowNode | null | undefined, key: string): string | undefined {
  const value = readFlowNodeData(node)[key]
  return typeof value === 'string' ? value : undefined
}

function readPointerClientPoint(evt: MouseEvent | TouchEvent): { x: number; y: number } | null {
...
const requestSilentProjectSave = () => {
...
const readUploadedAssetUrl = (asset: { data?: unknown } | null | undefined): string | null => {
  const data = readObjectRecord(asset?.data)
  if (!data) return null
  return normalizeImageUrl(data.url) || normalizeImageUrl(data.imageUrl) || normalizeImageUrl(data.thumbnailUrl)
}
...
const isCanvasReferencePickerCandidateNode = (node: FlowNode, targetNode: FlowNode | null): boolean => {
  if (!targetNode || node.id === targetNode.id || node.type !== 'taskNode') return false
  const data = (node.data ?? {}) as Record<string, unknown>
  const kind = typeof data.kind === 'string' ? data.kind : undefined
  const sourceCoreType = getTaskNodeCoreType(kind)
  const targetData = (targetNode.data ?? {}) as Record<string, unknown>
  const targetKind = typeof targetData.kind === 'string' ? targetData.kind : undefined
  const targetCoreType = getTaskNodeCoreType(targetKind)
  if (sourceCoreType === 'image') return Boolean(resolveNodePrimaryImageUrl(node))
  if (sourceCoreType === 'video' && targetCoreType === 'video') return Boolean(pickVideoTailFrameFromNode(node))
  return false
}
...
const CANVAS_CONTEXT_ADDABLE_KINDS = ['text', 'image', 'video', 'webHero'] as const
const NODE_VISIBILITY_FILTERS = ['text', 'image', 'video', 'web'] as const
...
const getNodeVisibilityIcon = (filter: NodeVisibilityFilter) => {
  ...
  if (filter === 'web') return IconWorldWww
  return IconVideo
}
...
function CanvasInner({ className }: CanvasInnerProps): JSX.Element {
  ...
  const pendingInitialView = useUIStore(s => s.pendingInitialView)
  const setPendingInitialView = useUIStore(s => s.setPendingInitialView)
  ...
  const autoLayoutCanvas = useRFStore(s => s.autoLayoutAllDagVertical)
  ...
  const [jarvisHubConnectSource, setJarvisHubConnectSource] = useState<{ nodeId: string } | null>(null)
  ...
  const viewOnlyFittedOnceRef = useRef(false)
  ...
  const importImagesFromFiles = useCallback(async (files: File[], basePosFlow?: { x: number; y: number }) => {
    ...
    const newNodes = prepared.map(({ id, label, localUrl, position }) => ({
      ...
      position,
        data: {
          ...
          kind: 'image',
          imageUrl: localUrl,
          imageResults: [{ url: localUrl, title: label }],
          imagePrimaryIndex: 0,
          nodeWidth: 280,
          nodeHeight: 280,
      },
    ...
    useRFStore.getState().appendImportedNodes(newNodes)

    const { patchNodeDataWithoutHistory } = useRFStore.getState()
    let successCount = 0
    ...
    for (const { id, file, localUrl, label } of prepared) {
      try {
        ...
        try {
          ...
          const url = readUploadedAssetUrl(hosted) || ''
          if (url) {
            hostedUrl = url
          ...
          const msg = formatErrorMessage(error).trim()
          ...
          if (mayHaveSucceeded) {
            ...
            const recoveredUrl = readUploadedAssetUrl(recovered) || ''
        ...
        patchNodeDataWithoutHistory(id, {
          imageUrl: bestUrl,
          imageResults: [{ url: bestUrl, title: label }],
          imagePrimaryIndex: 0,
  ...
  const importImageNodeFromDraggedFrame = useCallback(async (
    ...
    if (!preferred.startsWith('blob:')) {
      ...
      const node = {
        ...
        data: {
          ...
          kind: 'image',
          imageUrl: preferred,
          imageResults: [{ url: preferred, title: label }],
          imagePrimaryIndex: 0,
          nodeWidth: 280,
          nodeHeight: 280,
        },
      ...
      useRFStore.getState().appendImportedNodes([node])
      requestSilentProjectSave()
      return
    }
    ...
    try {
      ...
      patchNodeDataWithoutHistory(nodeId, {
        imageUrl: bestUrl,
        imageResults: [{ url: bestUrl, title: label }],
        imagePrimaryIndex: 0,
        serverAssetId: hostedAssetId,
  ...
  useEffect(() => {
    ...
    return () => {
	  ...
	  useEffect(() => {
	    const focusWindow = window as CanvasFocusWindow
	    focusWindow.__tcFocusNode = (nodeId: string) => {
      try {
        if (!nodeId) return
	    ...
	    return () => {
	      try {
	        if (focusWindow.__tcFocusNode) delete focusWindow.__tcFocusNode
	      } catch {
        // ignore
      }
    }
  ...
  useEffect(() => {
    ...
    if (pendingInitialView.kind === 'viewport') {
      rf.setViewport?.(pendingInitialView.value, { duration: 0 })
    } else {
      rf.fitView?.({ padding: 0.2 })
    }
    setPendingInitialView(null)
    viewInitializedRef.current = true
  }, [pendingInitialView, nodes.length, rf, setPendingInitialView])
  ...
  // dragging to a selector and can make nodes appear "undraggable". Strip it on mount.
	  useEffect(() => {
	    useRFStore.setState((s) => {
	      const hasDragHandle = (s.nodes || []).some(hasFlowNodeDragHandle)
	      if (!hasDragHandle) return {}
	      const nodes = (s.nodes || []).map(stripFlowNodeDragHandle)
	      return { nodes }
	    })
  }, [])
  ...
  const onDragOver = useCallback((evt: React.DragEvent) => {
    ...
    const hasJarvisHubImage = types.includes('application/jarvishub-image-url') || types.includes('application/jarvishub-frame-sample')
    evt.dataTransfer.dropEffect = (hasFiles || hasJarvisHubImage) ? 'copy' : 'move'
  }, [])
  ...
  const onDrop = useCallback((evt: React.DragEvent) => {
    ...
    setJarvisHubConnectSource(null)
    connectFromRef.current = null
    didConnectRef.current = false
    lastReason.current = null
    const tplName = evt.dataTransfer.getData('application/jarvishub-template')
    const rfdata = evt.dataTransfer.getData('application/reactflow')
    const flowRef = evt.dataTransfer.getData('application/jarvishub-flow')
    const jarvisHubImageUrl = evt.dataTransfer.getData('application/jarvishub-image-url')
    const jarvisHubFrameSample = evt.dataTransfer.getData('application/jarvishub-frame-sample')
    ...
    if (jarvisHubFrameSample) {
      try {
        const payload = JSON.parse(jarvisHubFrameSample) as { url?: string; remoteUrl?: string | null; time?: number }
    ...
    if (jarvisHubImageUrl) {
      const payload = getJarvisHubImageDragPayload(evt.dataTransfer)
  ...
  const isCompatibleTargetHandle = useCallback((meta: { targetNodeId: string; targetHandleId: string }, opts?: { silent?: boolean }) => {
    ...
    const sKind = getNodeDataString(sNode, 'kind')
    const tKind = getNodeDataString(tNode, 'kind')
  ...
  const handleConnect = useCallback((c: Connection) => {
    ...
    const routedConnection: RoutedConnection = {
      ...c,
      type: edgeRoute === 'orth' ? 'orth' : 'typed',
    }
    onConnect(routedConnection)
  }, [edgeRoute, onConnect])

  const onConnectStart = useCallback<OnConnectStart>((_evt, params) => {
    didConnectRef.current = false
    if (jarvisHubConnectSource) {
      setJarvisHubConnectSource(null)
    }
  ...
  const onNodeContextMenu = useCallback<NodeMouseHandler<FlowNode>>((evt, node) => {
    evt.preventDefault()
    if (suppressContextMenuRef.current) {
      suppressContextMenuRef.current = false
      return
  ...
  const onEdgeContextMenu = useCallback<EdgeMouseHandler<FlowEdge>>((evt, edge) => {
    evt.preventDefault()
    if (suppressContextMenuRef.current) {
      suppressContextMenuRef.current = false
      return
  ...
  type DragTrackingState = {
    snapIndex: DragSnapIndex
    nodeMetaById: Map<string, DragNodeMeta>
  }
  type IoDragBounds = {
    groupId: string
    maxX: number
    maxY: number
    positionKey: 'ioInPos' | 'ioOutPos'
  }
  type PendingIoDragPosition = {
    groupId: string
    positionKey: 'ioInPos' | 'ioOutPos'
  ...
  const dragNodeMetaByIdRef = useRef<Map<string, DragNodeMeta>>(new Map())
  const ioDragBoundsRef = useRef<IoDragBounds | null>(null)
  const pendingIoDragPositionRef = useRef<PendingIoDragPosition | null>(null)
  ...
  const readNodeDimension = (value: unknown, fallback: number): number => {
  ...
  const buildDragTrackingState = (allNodes: FlowNode[], excludeIds: Set<string>): DragTrackingState => {
    const xs: number[] = []
    const ys: number[] = []
    const nodeMetaById = new Map<string, DragNodeMeta>()
    ...
    xs.sort((a, b) => a - b)
    ys.sort((a, b) => a - b)
    return { snapIndex: { xs, ys }, nodeMetaById }
  }

  const setDragSnapIndex = (excludeIds: Set<string>) => {
    dragExcludeIdsRef.current = excludeIds
    const trackingState = buildDragTrackingState(useRFStore.getState().nodes, excludeIds)
    dragSnapIndexRef.current = trackingState.snapIndex
    dragNodeMetaByIdRef.current = trackingState.nodeMetaById
  ...
  const ensureDragSnapIndex = (excludeIds: Set<string>) => {
    ...
    const next = buildDragTrackingState(useRFStore.getState().nodes, excludeIds)
    dragSnapIndexRef.current = next.snapIndex
    dragNodeMetaByIdRef.current = next.nodeMetaById
    dragSnapExcludeKeyRef.current = key
    return next.snapIndex
  }

  const onNodeDragStart = useCallback<OnNodeDrag<FlowNode>>((_evt, node) => {
    setDragging(true)
    resourceManager.setNodeDragging(true)
    pendingIoDragPositionRef.current = null
    ioDragBoundsRef.current = null
    const id = typeof node?.id === 'string' ? node.id : null
    const excludeIds = buildExcludeIdsForDragStart(id)
    setDragSnapIndex(excludeIds)
    const parentId = typeof node?.parentId === 'string' ? node.parentId : null
    if (node?.type === 'ioNode' && parentId) {
      const groupNode = useRFStore.getState().nodes.find((candidate) => candidate.id === parentId)
      if (groupNode) {
        const styleRecord = readObjectRecord(groupNode.style)
        const groupWidth = readNodeDimension(groupNode.width, readNodeDimension(styleRecord?.width, 240))
        const groupHeight = readNodeDimension(groupNode.height, readNodeDimension(styleRecord?.height, 160))
        const nodeData = readObjectRecord(node?.data)
        ioDragBoundsRef.current = {
          groupId: parentId,
          maxX: Math.max(0, groupWidth - 96),
          maxY: Math.max(0, groupHeight - 28),
          positionKey: nodeData?.kind === 'io-in' ? 'ioInPos' : 'ioOutPos',
        }
      }
    }
  }, [])

  const onNodeDrag = useCallback<OnNodeDrag<FlowNode>>((_evt, node) => {
    ...
    // Persist IO-node relative position once on drag stop; avoid full-node store writes per frame.
    const ioBounds = ioDragBoundsRef.current
    if (node?.type === 'ioNode' && ioBounds) {
      const positionX = Number(node?.position?.x)
      const positionY = Number(node?.position?.y)
      if (Number.isFinite(positionX) && Number.isFinite(positionY)) {
        pendingIoDragPositionRef.current = {
          groupId: ioBounds.groupId,
          positionKey: ioBounds.positionKey,
          rel: {
            x: clampPosition(positionX, 0, ioBounds.maxX),
            y: clampPosition(positionY, 0, ioBounds.maxY),
          },
        }
      }
    }
  ...
  const onNodeDragStop = useCallback((_event: React.MouseEvent | React.TouchEvent, _node: FlowNode) => {
    const pendingIoDragPosition = pendingIoDragPositionRef.current
    ...
    dragNodeMetaByIdRef.current = new Map()
    ioDragBoundsRef.current = null
    pendingIoDragPositionRef.current = null
    if (pendingIoDragPosition) {
      useRFStore.setState((s) => ({
        nodes: s.nodes.map((n) =>
          n.id === pendingIoDragPosition.groupId
            ? { ...n, data: { ...n.data, [pendingIoDragPosition.positionKey]: pendingIoDragPosition.rel } }
            : n,
        ),
      }))
    }
  ...
  const handleNodesChange = useCallback((changes: NodeChange<FlowNode>[]) => {
    ...
    const threshold = 6
    const positionChanges = normalizedChanges.filter((ch): ch is NodePositionChange => ch?.type === 'position' && Boolean(ch.position))
    ...
    const draggingTick = positionChanges.some((ch) => ch.dragging === true)
    const index = draggingTick
      ...
      : buildDragTrackingState(useRFStore.getState().nodes, movedIds).snapIndex
    const stateNodesById = draggingTick
      ? null
      : new Map(useRFStore.getState().nodes.map((n) => [n.id, n] as const))
    const snapped = normalizedChanges.map((ch) => {
      if (ch.type === 'position' && ch.position) {
        const node = typeof ch.id === 'string'
          ? draggingTick
            ? dragNodeMetaByIdRef.current.get(ch.id)
            : stateNodesById?.get(ch.id)
          : undefined
        const isGroupNode = node?.type === 'groupNode'
        const isChildNode = Boolean(node?.parentId)
    ...
    const compacted = snapped.filter((ch) => {
      ...
      const draggingFlag = ch.dragging
  ...
  useEffect(() => {
    ...
    if (jarvisHubConnectSource && !hasNode(jarvisHubConnectSource.nodeId)) {
      setJarvisHubConnectSource(null)
      setConnectingType(null)
    }
  ...
  }, [clearSnapTarget, nodes, jarvisHubConnectSource])
      ...
      <g className="tc-connection-line">
        ...
        {snapTarget && (
          <circle
            ...
            cy={ty}
            r={4}
            fill={String(props.connectionLineStyle?.stroke || '#339CFF')}
            opacity={0.9}
          />
        )}
      </g>
  ...
  const quickConnectNodes = useCallback((sourceId: string, targetId: string, opts?: { showInvalidToast?: boolean }) => {
    ...
    const sKind = getNodeDataString(sourceNode, 'kind')
    const tKind = getNodeDataString(targetNode, 'kind')
  ...
  const onNodeClick = useCallback<NodeMouseHandler<FlowNode>>((evt, node) => {
    if (!node?.id) return
    if (referencePickerTargetId) {
      if (node.id === referencePickerTargetId) return
      const referencePickerTargetNode = nodes.find((item) => item.id === referencePickerTargetId) as FlowNode | undefined
      if (!isCanvasReferencePickerCandidateNode(node, referencePickerTargetNode ?? null)) return
    ...
    const pending = jarvisHubConnectSource
    ...
    const kind = String(readFlowNodeData(node).kind || '').toLowerCase()
    const derivedType =
      kind === 'image' ? 'image'
      : kind === 'video' ? 'video'
      : kind === 'text' ? 'text'
      : null
    setJarvisHubConnectSource({ nodeId: node.id })
    setConnectingType(derivedType)
  }, [closeCanvasReferencePicker, nodes, quickConnectNodes, referencePickerBlockedSourceIds, referencePickerTargetId, jarvisHubConnectSource])

  const onNodeDoubleClick = useCallback((evt: React.MouseEvent, node: FlowNode) => {
    if (!node?.id) return
    if (node.type === 'groupNode') return
    // Ignore double-clicks that originated inside a pptDeck navigation hot
    // zone or any element that explicitly opted out of node interactions via
    // `nodrag`. Without this guard, a fast double-tap on the prev/next zones
    // would bubble up here and focus into the deck's subgraph mid-navigation.
    const target = evt.target as HTMLElement | null
    if (target && typeof target.closest === 'function') {
      if (target.closest('.tc-ppt-deck-single__nav-zone')) return
      if (target.closest('.tc-ppt-deck-single__nav')) return
    }
  ...
  const collectGroupSubgraph = useCallback((groupId: string): { nodes: FlowNode[]; edges: FlowEdge[]; groupLabel: string } | null => {
    ...
    const groupLabel = String(readFlowNodeData(rootGroup).label || groupId).trim() || groupId
  ...
  const resolveSubgraphPreviewImageUrl = useCallback((groupId: string): string | null => {
    ...
    for (const node of subgraph.nodes) {
      const imageUrl = resolveNodePrimaryImageUrl(node)
  ...
  useEffect(() => {
    ...
    setWorkflowNameInput((selectedProject.name || '').trim())
    setWorkflowDescriptionInput('')
    setWorkflowCoverUrlInput('')
    setTemplateVisibility('private')
  ...
  const loadImageFromBlob = useCallback((blob: Blob): Promise<HTMLImageElement> => new Promise((resolve, reject) => {
    ...
    const ImageCtor = (typeof window !== 'undefined' ? window.Image : (globalThis as GlobalImageProvider).Image) as
  ...
  const stitchGroupToLongImage = useCallback(async (groupId: string) => {
    ...
    try {
      ...
      const groupLabel = String(readFlowNodeData(group).label || groupId).trim() || groupId
  ...
  const styledViewNodes = useMemo(() => {
    ...
    const referencePickerTargetNode = referencePickerTargetId
      ? focusFiltered.nodes.find((node) => node.id === referencePickerTargetId) ?? null
      : null
    return focusFiltered.nodes.map((node) => {
      const isReferencePickerCandidate = Boolean(
        referencePickerTargetNode && isCanvasReferencePickerCandidateNode(node, referencePickerTargetNode),
      )
  ...
  const orthEdgeObstacles = useMemo<OrthEdgeObstacle[]>(() => {
    const nodesById = new Map(styledViewNodes.map((node) => [node.id, node] as const))
    return styledViewNodes.map((node) => {
      const pos = getNodeAbsPosition(node, nodesById)
      const { w, h } = getNodeSize(node, { w: 180, h: 96 })
      return { x: pos.x, y: pos.y, w, h, id: node.id }
    })
  }, [styledViewNodes])
  ...
  const viewEdges = useMemo(() => {
    ...
    return routed.map((e) => {
      ...
      const relationship = readCanvasEdgeRelationship(e.data)
      return {
        ...e,
        style: {
          ...(e.style || {}),
          opacity: getCanvasEdgeOpacity({
            relationship,
            connectedToSelection: active,
            mutedByVisibility: isMuted,
          }),
          ...(active && !isMuted ? { stroke: isDarkCanvas ? '#e5e7eb' : '#111827' } : {}),
  ...
  const handleInsertNodeAt = (
    ...
    const upstreamNode = menuState.fromNodeId
      ...
      : undefined
    const upstreamPrompt = getNodeDataString(upstreamNode, 'prompt')
    const sourceKind = getNodeDataString(upstreamNode, 'kind')

    useRFStore.setState(s => {
      const id = genTaskNodeId()
      const schema = getTaskNodeSchema(targetKind)
      const label = schema.label || schema.kind || 'Node'
      const data: Record<string, unknown> = { label, kind: schema.kind }
  ...
  const insertMenuContent = useMemo(() => {
    ...
    const schemaCandidates = listTaskNodeSchemas()
      ...
      .sort((a, b) => {
        const order: Record<string, number> = { image: 10, video: 20, document: 30, generic: 100 }
  ...
  // Share/view-only: fit the persisted graph once after initial load without changing node positions.
  useEffect(() => {
    if (!viewOnly) {
      viewOnlyFittedOnceRef.current = false
      return
    }
    if (viewOnlyFittedOnceRef.current) return
    if (viewInitializedRef.current) return
    if (!nodes.length) return
    viewOnlyFittedOnceRef.current = true
  ...
  return (
    <CanvasRenderContext.Provider value={canvasRenderContextValue}>
      <OrthEdgeObstaclesContext.Provider value={orthEdgeObstacles}>
      <div className={joinClassNames('tc-canvas', className)}
        style={canvasStyle}
        data-connecting={connectingType || ''}
        data-connecting-active={(isConnecting || !!jarvisHubConnectSource) ? 'true' : 'false'}
        ...
        onPaste={(e) => {
          ...
          if (filesFromClipboard.length) {
            e.preventDefault()
            e.stopPropagation()
            ;(window as CanvasFocusWindow).__tcLastImagePasteAt = Date.now()
          ...
          if (text) {
            try {
              ...
              if (extracted?.nodes.length) {
                e.preventDefault()
                e.stopPropagation()
                ;(window as CanvasFocusWindow).__tcLastWorkflowPasteAt = Date.now()
      ...
      <ReactFlow className="tc-canvas__flow"
        ...
        // Shift is reserved for the group-resize "scale children" modifier (see GroupNode).
        // selectionOnDrag already provides box-select without a key; null disables Shift's
        // box-select binding so it doesn't intercept the resize handle's drag.
        selectionKeyCode={null}
        // Edit mode: middle-button and right-button drag pan the canvas; left drag keeps selection box.
        panOnDrag={viewOnly ? true : [1, 2]}
        panOnScroll
        zoomOnPinch
        zoomOnScroll
        minZoom={MIN_ZOOM}
            ...
            <Group className="tc-canvas__visibility-panel-group" gap={4} wrap="nowrap">
              ...
              <Divider
                ...
                style={{
                  height: 18,
                  alignSelf: 'center',
                  borderColor: isDarkCanvas ? 'rgba(148, 163, 184, 0.18)' : 'rgba(15, 23, 42, 0.1)',
                }}
              />
              <Button
                className="tc-canvas__visibility-panel-tag"
                ...
                radius="xs"
                variant="subtle"
                leftSection={<IconLayoutGridAdd className="tc-canvas__visibility-panel-tag-icon" size={12} stroke={1.9} />}
                styles={{
                  root: {
                    height: 24,
                    paddingInline: 8,
                    border: `1px solid ${isDarkCanvas ? 'rgba(148, 163, 184, 0.22)' : 'rgba(15, 23, 42, 0.1)'}`,
                    background: isDarkCanvas ? 'rgba(51, 65, 85, 0.58)' : 'rgba(248, 250, 252, 0.9)',
                    color: isDarkCanvas ? 'rgba(241, 245, 249, 0.92)' : 'rgba(15, 23, 42, 0.84)',
                    transition: 'all 160ms ease',
                  },
                  section: {
                    marginRight: 4,
      ...
      {!viewOnly && focusedNodeSummary && (() => {
        ...
        const breadcrumb = (
          <PanelCard
            ...
            style={{
              position: slot ? 'relative' : 'absolute',
              left: slot ? undefined : '50%',
              top: slot ? undefined : 12,
              transform: slot ? undefined : 'translateX(-50%)',
          ...
          >
            <Group className="tc-canvas__breadcrumb-group" gap={8} style={{ flexWrap: 'nowrap' }}>
              ...
              <Button className="tc-canvas__breadcrumb-button" size="xs" variant="default">
      ...
      </OrthEdgeObstaclesContext.Provider>
