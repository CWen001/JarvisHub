...
export type GroupDownloadableMediaType = 'image' | 'video' | 'audio' | 'html'
...
function pickPrimaryHtmlUrl(data: UnknownNodeData): string | null {
  const direct = normalizeString(data.webHeroDocumentHtml)
  if (!direct) return null
  return `data:text/html;charset=utf-8,${encodeURIComponent(direct)}`
}
...
function pickPrimaryMedia(data: UnknownNodeData): { mediaType: GroupDownloadableMediaType; url: string } | null {
  ...
  const htmlUrl = pickPrimaryHtmlUrl(data)

  if (kind === 'webHero') {
    if (htmlUrl) return { mediaType: 'html', url: htmlUrl }
    if (videoUrl) return { mediaType: 'video', url: videoUrl }
    if (imageUrl) return { mediaType: 'image', url: imageUrl }
    return null
  }

  if (schema.category === 'image') {
    if (imageUrl) return { mediaType: 'image', url: imageUrl }
    if (videoUrl) return { mediaType: 'video', url: videoUrl }
  ...
  if (schema.features.includes('audio')) {
    if (audioUrl) return { mediaType: 'audio', url: audioUrl }
    if (videoUrl) return { mediaType: 'video', url: videoUrl }
    if (imageUrl) return { mediaType: 'image', url: imageUrl }
    return null
