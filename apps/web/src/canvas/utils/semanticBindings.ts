...
function parseRoleNameFromNote(note: string): string {
  ...
  if (prefix.startsWith('role:')) return prefix.slice('role:'.length).trim()
  return ''
}
...
function parseVisualNameFromNote(note: string): { name: string; kind: 'scene' | 'prop' | null } {
  ...
  if (prefix.startsWith('scene:')) {
    return {
      name: prefix.slice('scene:'.length).trim(),
      kind: 'scene',
    }
  }
  if (prefix.startsWith('prop:')) {
    return {
      name: prefix.slice('prop:'.length).trim(),
      kind: 'prop',
    }
  }
...
function inferRoleReferenceViewFromSource(data: Record<string, unknown>): SemanticReferenceView | null {
  ...
  if (source === 'asset_confirm' && readTrimmedString(data.roleName)) {
    return 'three_view'
  }
  return null
}
