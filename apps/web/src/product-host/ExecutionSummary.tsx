...
import { Collapse, Progress, Text, UnstyledButton } from '@mantine/core'
import { IconCheck, IconChevronDown, IconChevronUp, IconClock, IconProgress } from '@tabler/icons-react'
import type { LiveChatRunRecord } from '../ui/chat/liveChatRunStore'
import { resolveExecutionSummary, type ExecutionSummary as ExecutionSummaryView } from '../ui/chat/executionSummaryModel'
...
export function CompactExecutionSummary({
  summary,
  identity,
}: {
  ...
  if (summary.taskCount === 0) return null
    ...
    <section className="compact-execution" data-phase={summary.phase}>
      <div className="compact-execution__row">
        ...
        >
          ...
          <strong>{summary.phase === 'running' && summary.activeTaskLabel ? summary.activeTaskLabel : summary.headline}</strong>
...
export function ExecutionSummary({ run }: { run: LiveChatRunRecord | null }): JSX.Element | null {
  ...
  return <CompactExecutionSummary summary={summary} identity={run.runId} />
}
