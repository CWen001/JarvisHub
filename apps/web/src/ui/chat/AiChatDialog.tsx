...
function mergeAssistantAssetRecords(...groups: readonly AssistantAsset[][]): AssistantAsset[] {
...
function mergeAssistantAssets(
  ...
  const normalizedExtra = (Array.isArray(extraImages) ? extraImages : []).map((image, index) => ({
    title: String(image?.title || '').trim() || `生成图-${index + 1}`,
    url: String(image?.url || '').trim(),
    mediaType: 'image' as const,
  }))
  return mergeAssistantAssetRecords(base, normalizedExtra)
}
...
function normalizeAssistantAssets(input: unknown): AssistantAsset[] {
  ...
  for (const item of items) {
    ...
    const rawType = typeof record.mediaType === 'string'
      ? record.mediaType.trim().toLowerCase()
...
export function ChatBubble({
  ...
  if (
    ...
    visibleAssistantAssets.length === 0 &&
    !shouldShowRunLoader &&
    !message.turnVerdict
  ) {
    return null
      ...
      <PanelCard className="tc-ai-chat-bubble__card" padding="compact">
        ...
        {isUser ? (
          ...
          <>
            ...
            {message.kind === 'error' && onRetry ? (
              <Button
                className="tc-ai-chat-bubble__retry"
                size="compact-xs"
                variant="light"
                color="red"
                mt="sm"
        ...
        {!isUser && visibleAssistantAssets.length > 0 ? (
          <Group className="tc-ai-chat-bubble__assets" gap={8} mt={8} align="flex-start" wrap="wrap">
            {visibleAssistantAssets.map((asset, idx) => {
              const url = String(asset?.url || '').trim()
              if (!url) return null
...
function ChatRuntimeController({
  ...
  const {
    shouldAutoScrollRef,
    showJumpToLatest,
    syncFromViewport: syncAutoScrollPreference,
    resume: resumeAutoFollow,
  } = useTimelineAutoFollow(viewportRef)
  ...
  const attachSuccessfulMediaResult = React.useCallback((
    result: AgentsChatMediaResultStreamPayload,
    messageId: string,
    targetTabId: string,
  ) => {
    const asset = resolveSuccessfulMediaResultArtifact({
      result,
      nodes: useRFStore.getState().nodes,
    })
    if (!asset) return
    setMessages((prev) => patchChatMessageById(prev, messageId, (message) => {
      const assets = Array.isArray(message.assets) ? message.assets : []
      const existingIndex = assets.findIndex((candidate) => (
        String(candidate.nodeId || '').trim() === asset.nodeId
        || Boolean(asset.assetId && String(candidate.assetId || '').trim() === asset.assetId)
        || Boolean(asset.assetRefId && String(candidate.assetRefId || '').trim() === asset.assetRefId)
        || String(candidate.url || '').trim() === asset.url
      ))
      if (existingIndex < 0) return { ...message, assets: [...assets, asset].slice(0, 12) }
      const nextAssets = [...assets]
      nextAssets[existingIndex] = { ...nextAssets[existingIndex], ...asset }
      return { ...message, assets: nextAssets }
    }), targetTabId)
  }, [setMessages])
  ...
  const activeRunHasPersistedExecution = React.useMemo(() => {
    if (!activeLiveRun || activeLiveRun.status === 'running') return false
    const assistantMessageId = String(activeLiveRun.assistantMessageId || '').trim()
    if (!assistantMessageId) return false
    const message = visibleMessages.find((candidate) => candidate.id === assistantMessageId)
    const turns = message?.toolCallSnapshot?.record.toolCallsByTurn
    return Boolean(turns && Object.values(turns).some((calls) => calls.length > 0))
  }, [activeLiveRun, visibleMessages])
  ...
  }, [effectiveChatSessionKey, resumeAutoFollow])
    ...
    const finishRecoveredRun = async (
      ...
      setMessages((prev) =>
        patchChatMessageById(prev, messageId, (message) => ({
          ...message,
          content: reply,
          assets: mergeAssistantAssetRecords(
            normalizeAssistantAssets(message.assets),
            assistantAssets,
          ),
          ts: formatNowTime(),
          phase: 'final',
        ...
        { runId: activeRun.runId, afterSeq: 0 },
        {
          onEvent: (event) => {
            ...
            if (event.event === 'media_result') {
              ...
              attachSuccessfulMediaResult(event.data, messageId, requestTabId)
  ...
  const scrollToBottom = React.useCallback((reason: TimelineAutoFollowResumeReason = 'programmatic') => {
    const el = viewportRef.current
    if (!el) return
    try {
      el.scrollTop = el.scrollHeight
      resumeAutoFollow(reason)
    } catch {
      // ignore
    }
  }, [resumeAutoFollow])
        ...
        void agentsChatStream(requestPayload, {
          onEvent: (event) => {
            ...
            if (event.event === 'media_result') {
              ...
              attachSuccessfulMediaResult(event.data, pendingId, requestTabId)
  ...
  const retryFailedAssistantMessage = React.useCallback((messageId: string) => {
    if (isActiveTabSending) return
    const text = resolveNativeRetryText(messages, messageId)
    if (!text) {
      toast('未找到可重试的原始请求', 'error')
      return
    }
    void send({ text, displayText: text, attachCanvasContext: true })
  }, [isActiveTabSending, messages, send])
