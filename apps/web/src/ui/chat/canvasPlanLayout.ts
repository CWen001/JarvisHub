...
// Sizes resolved via canvas/nodeSizes — keep this file's fallback aligned with the single source of truth.
const DEFAULT_NODE_SIZE = estimateNodeRenderSize({})
...
function estimateNodeSize(node: LayoutPlanNode): { w: number; h: number } {
  ...
  // composeVideo is a video-family kind not enumerated in TaskNodeCoreType — coerce it manually.
...
function applyGridLayout(nodes: LayoutPlanNode[]): SizedNode[] {
  ...
  const sizes = sorted.map((node) => estimateNodeSize(node))
  // Cell width = widest node in that column; cell height = tallest node in that row.
  // Otherwise video nodes (480+ wide) end up overlapping narrow image nodes in adjacent columns.
  const colWidths = new Array<number>(cols).fill(DEFAULT_NODE_SIZE.w)
  const rowHeights = new Array<number>(rows).fill(DEFAULT_NODE_SIZE.h)
  for (let i = 0; i < sorted.length; i += 1) {
    const col = i % cols
    const row = Math.floor(i / cols)
    const size = sizes[i] ?? DEFAULT_NODE_SIZE
  ...
  for (let c = 0; c < cols; c += 1) {
    ...
    cursorX += (colWidths[c] ?? DEFAULT_NODE_SIZE.w) + INTERNAL_GAP_X
  }
  const rowOffsets: number[] = []
  let cursorY = 0
  for (let r = 0; r < rows; r += 1) {
    rowOffsets.push(cursorY)
    cursorY += (rowHeights[r] ?? DEFAULT_NODE_SIZE.h) + INTERNAL_GAP_Y
  }
  return sorted.map((node, index) => {
    ...
    return {
      ...node,
      layoutSize: sizes[index] ?? estimateNodeSize(node),
      position: {
        x: colOffsets[col] ?? 0,
        y: rowOffsets[row] ?? 0,
      },
...
function applyHybridLaneLayout(nodes: LayoutPlanNode[]): SizedNode[] {
  const lanesByPriority = new Map<number, LayoutPlanNode[]>()
  for (const node of nodes) {
    const priority = getNodePriority(node)
    const lane = lanesByPriority.get(priority) ?? []
    lane.push(node)
    lanesByPriority.set(priority, lane)
  }
  const lanes = Array.from(lanesByPriority.entries())
...
function layoutGroupNodes(nodes: LayoutPlanNode[], edges: LayoutPlanEdge[]): SizedNode[] {
  if (!hasDenseOrMissingPositions(nodes)) {
    return normalizeRelativePositions(nodes)
  }
