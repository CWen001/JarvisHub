...
export function formatTurnVerdictSummary(
  ...
  const reasonSet = new Set(verdict.reasons)
  const hasSpecificMediaReason =
    reasonSet.has('external_media_generation_waiting') ||
    reasonSet.has('media_wait_timeout') ||
    reasonSet.has('media_generation_blocked')
  const displayReasons = hasSpecificMediaReason
    ? verdict.reasons.filter(
      (reason) => reason !== 'tool_execution_issues' && reason !== 'diagnostic_flags_present',
    )
    : verdict.reasons
  const labels = displayReasons.map((reason) => {
    switch (reason) {
      ...
      case 'external_media_generation_waiting':
        return '等待外部生成完成'
      case 'media_wait_timeout':
        return '外部生成等待超时'
      case 'media_generation_blocked':
        return '媒体生成已阻塞'
      default:
        return reason
    }
