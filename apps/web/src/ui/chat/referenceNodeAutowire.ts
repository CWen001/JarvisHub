...
export function collectSourceUrls(node: Node): string[] {
...
export function resolveSourceHandle(node: Node): SourceHandleId | null {
  const data = asRecord(node.data)
  const kind = typeof data?.kind === 'string' ? data.kind : null
  const coreType = getTaskNodeCoreType(kind)
  if (coreType === 'image') return 'out-image'
...
function resolveTargetHandle(node: Node): TargetHandleId | null {
  ...
  const kind = typeof data?.kind === 'string' ? data.kind : null
  const coreType = getTaskNodeCoreType(kind)
  if (coreType === 'image') return 'in-image'
