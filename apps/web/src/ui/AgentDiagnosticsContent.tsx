...
import { ToolCallsCoworkView } from './chat/ToolCallsCoworkView'
import { buildLiveRecordFromTrace } from './chat/traceToLiveRecord'
import { TodoProgressCard } from './chat/TodoProgressCard'
import type { ChatTodoItem } from './chat/chatTodoTypes'
...
export default function AgentDiagnosticsContent(props: AgentDiagnosticsContentProps): JSX.Element {
  ...
  }, [opened, projectId, label, workflowKey, publicChatVerdict, publicChatOutcome])
    ...
    <Stack className={className} gap="md">
      {activeLiveRun ? (
        <Stack className="agent-diagnostics-panel-live-run" gap={6} p="sm" style={{ border: '1px solid var(--mantine-color-blue-4)', borderRadius: 10, background: 'rgba(34, 139, 230, 0.06)' }}>
          ...
          {activeLiveRun.todoItems.length > 0 ? (
            <TodoProgressCard
              items={activeLiveRun.todoItems.map<ChatTodoItem>((item) => ({
                status: item.status,
      ...
      <ScrollArea className="agent-diagnostics-panel-scroll" h={520} offsetScrollbars>
        <Stack className="agent-diagnostics-panel-content" gap="sm">
          ...
          {!loading && (!data || (data.traces.length === 0 && data.publicChatRuns.length === 0)) ? (
                ...
                <Collapse in={isExpanded}>
                  <Stack className="agent-diagnostics-panel-trace-details" gap={6} mt="xs">
                    ...
                    {todoEvents.length > 0 ? (
                      <Stack className="agent-diagnostics-panel-trace-todo-events-block" gap={4}>
                        ...
                        {todoEvents.map((event, index) => {
                          const eventStatus = getTodoEventTimelineStatus(event)
                          return (
                    ...
                    {Array.isArray(item.toolCalls) && item.toolCalls.length > 0 ? (() => {
                      const adapted = buildLiveRecordFromTrace(item.toolCalls)
                      return (
                        <div className="agent-diagnostics-panel-trace-tools">
                          <ToolCallsCoworkView
                            turnIds={adapted.turnIds}
...
function buildTraceTimelineItems(input: {
  ...
  for (const event of input.todoEvents) {
    items.push({
      ...
      status: getTodoEventTimelineStatus(event),
...
function readTodoTraceItems(value: unknown): TodoTraceItem[] {
  ...
  for (const entry of value) {
    ...
    const waitingCount = readTodoCount(record.waitingCount, items.filter((item) => item.status === 'waiting').length)
    const blockedCount = readTodoCount(record.blockedCount, items.filter((item) => item.status === 'blocked').length)
    const pendingCount = readTodoCount(record.pendingCount, items.filter((item) => item.status === 'pending').length)
    out.push({
      sourceToolCallId,
      items,
      totalCount,
...
function buildTodoEventTitle(event: TodoTraceItem): string {
  const blockedItem = event.items.find((item) => item.status === 'blocked') ?? null
  if (blockedItem) return `todo blocked · ${blockedItem.text}`
  const activeItem = event.items.find((item) => item.status === 'in_progress') ?? null
  if (activeItem) return `todo update · ${activeItem.text}`
  const waitingItem = event.items.find((item) => item.status === 'waiting') ?? null
  if (waitingItem) return `todo waiting · ${waitingItem.text}`
  return `todo update · ${event.completedCount}/${event.totalCount} completed`
}

export type TodoEventTimelineStatusInput = {
  inProgressCount: number
  waitingCount: number
  blockedCount: number
  pendingCount: number
...
export function getTodoEventTimelineStatus(event: TodoEventTimelineStatusInput): string {
  if (event.blockedCount > 0) return 'blocked'
  if (event.inProgressCount > 0) return 'in_progress'
  if (event.waitingCount > 0) return 'waiting'
