...
import type { ProjectCanvasOwnerType } from '../projects/projectCanvasEntry'

let hoveredEdgeClearTimer: ReturnType<typeof setTimeout> | null = null

const PUBLIC_API_KEY_STORAGE_KEY = 'canvas_public_api_key'
const PUBLIC_VENDOR_CANDIDATES_STORAGE_KEY = 'canvas_public_vendor_candidates'
const AI_CHAT_WATCH_ASSETS_STORAGE_KEY = 'canvas_ai_chat_watch_assets'

function getInitialAssetPersistence(): boolean {
  if (typeof window === 'undefined') return true
  try {
    const v = window.localStorage.getItem('canvas_asset_persist')
    if (v === '0') return false
  } catch {
    // ignore
  }
...
export type CreationUnitType = 'scene' | 'shot' | 'clip'
...
export type PendingInitialView =
  | { kind: 'viewport'; value: { x: number; y: number; zoom: number } }
  | { kind: 'fit' }
  | null

export type CurrentFlowState = {
  id?: string | null
  name: string
  source: 'local' | 'server'
  ownerType?: ProjectCanvasOwnerType | null
  ownerId?: string | null
...
type UIState = {
  ...
  activePanel: 'add' | 'gallery' | 'project' | 'models' | 'history' | 'memory' | null
  setActivePanel: (p: 'add' | 'gallery' | 'project' | 'models' | 'history' | 'memory' | null) => void
  ...
  setCurrentFlow: (patch: Partial<CurrentFlowState>) => void
  canvasViewport: { x: number; y: number; zoom: number } | null
  setCanvasViewport: (v: { x: number; y: number; zoom: number } | null) => void
  pendingInitialView: PendingInitialView
  setPendingInitialView: (v: PendingInitialView) => void
  isDirty: boolean
...
function normalizePersistedCreationSession(value: unknown): PersistedCreationSession | null {
  ...
  const unitType: CreationUnitType =
    unitTypeRaw === 'shot' || unitTypeRaw === 'clip'
      ? unitTypeRaw
      : 'scene'
...
export const useUIStore = create<UIState>((set, get) => ({
  ...
  currentFlow: { id: null, name: '未命名', source: 'local', ownerType: 'project', ownerId: null, updatedAt: null },
  ...
  setPendingInitialView: (v) => set({ pendingInitialView: v }),
  isDirty: false,
  ...
  setAssetPersistenceEnabled: (v) => {
    ...
    if (typeof window !== 'undefined') {
      try {
        window.localStorage.setItem('canvas_asset_persist', v ? '1' : '0')
      } catch {
        // ignore
      }
    }
