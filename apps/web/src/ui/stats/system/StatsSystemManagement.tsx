...
import { ActionIcon, Badge, Button, CopyButton, Divider, Group, Loader, Menu, Modal, NumberInput, Select, Stack, Switch, Table, Text, Textarea, Tooltip, Title, UnstyledButton } from '@mantine/core'
import { IconCheck, IconChevronRight, IconCopy, IconEye, IconPlayerPlay, IconRefresh, IconRestore, IconUpload } from '@tabler/icons-react'
import { API_BASE, fetchPublicTaskResultWithAuth, getPromptEvolutionRuntime, listPromptEvolutionRuns, listTaskLogs, publishPromptEvolutionRun, rollbackPromptEvolution, runPromptEvolution, type PromptEvolutionRunHistoryDto, type PromptEvolutionRunResponseDto, type PromptEvolutionRuntimeDto, type TaskAssetDto, type TaskKind, type VendorCallLogDto, type VendorCallLogStatus } from '../../../api/server'
...
type StatsSystemGroupKey = 'models' | 'diagnostics' | 'publicApi' | 'assets'
...
const STATS_SYSTEM_NAV_GROUPS: StatsSystemNavGroup[] = [
  ...
  },
  {
    key: 'publicApi',
    label: '公开端点',
    description: '外站调用地址与在线调试。',
    children: [
      { value: 'apiAccess', label: 'API 接入', description: '查看公开调用地址和在线调试。' },
    ],
  },
  {
    key: 'assets',
...
const STATS_SYSTEM_SECTION_META: Record<StatsSystemSection, { label: string; description: string; groupKey: StatsSystemGroupKey }> = {
  ...
  apiAccess: { label: 'API 接入', description: '查看公开调用地址和在线调试。', groupKey: 'publicApi' },
...
export default function StatsSystemManagement({ className }: { className?: string }): JSX.Element {
  ...
  const [section, setSection] = React.useState<StatsSystemSection>('modelCatalog')
...
fetch('${publicDrawUrl}', {
  ...
  body: JSON.stringify({
    vendor: 'auto',
    prompt: '一个赛博风格的透明玻璃徽章，中文“JarvisHub”，高细节，干净背景',
