...
const SKILL_JSON_TAG = 'canvas_skill_json'
...
function buildSkillFromVisionSystemPrompt(): string {
  return [
    '你是 JarvisHub 后台运营的「Skill 构建助手」。',
    '- 全程用中文输出，不要询问用户选择语言。',
    '- 输入会包含参考作品的视觉拆解（JSON 或文字）。',
    '- 你的目标：生成一段可复用的 Skill 文本（会拼接进 system prompt），用于稳定复刻该作品的视觉风格与优点。',
    '- 必须输出严格 JSON，并用 <canvas_skill_json>...</canvas_skill_json> 包裹；除此之外不要输出任何文字。',
    'JSON 字段：',
...
function buildSkillFromVisionUserPrompt(input: { visionText: string; workUrl?: string; notes?: string }): string {
  ...
  return [
    ...
    vision,
    '',
    '请基于以上信息输出 <canvas_skill_json> 严格 JSON。',
  ].filter(Boolean).join('\n\n')
}
  ...
  const runGenerator = React.useCallback(async () => {
    ...
    try {
      ...
      const agentReply = await (async (): Promise<string> => {
        try {
          const resp = await agentsChat({
            vendor: 'agents',
            prompt,
            systemPrompt,
            temperature: 0.2,
            userMessageId: `m_user_stats_${crypto.randomUUID()}`,
            assistantMessageId: `m_ai_stats_${crypto.randomUUID()}`,
          })
            ...
            <Stack className="stats-agents-management__skill-generator" gap="sm" mt="sm">
              ...
              {generatorAgentPreview ? (
                <Textarea
                  ...
                  label="中间结果：agents-cli 输出（含 <canvas_skill_json>）"
                  autosize
                  minRows={4}
                  maxRows={10}
