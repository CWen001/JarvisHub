...
import StatusOverview from '../../../modelPanel/StatusOverview'
...
export default function StatsModelCatalogManagement({ className }: { className?: string }): JSX.Element {
  ...
  const [modelConfig, setModelConfig] = React.useState<ModelConfigDto | null>(null)
  const [defaultSavingSlot, setDefaultSavingSlot] = React.useState<ModelConfigDefaultSlot | null>(null)
  ...
  const resolveTaskKindsForModelKind = React.useCallback((kind: ModelCatalogModelKind): ProfileKind[] => {
  ...
  const reloadAll = React.useCallback(async () => {
    setLoading(true)
    try {
      const [v, m, mp, cfg] = await Promise.allSettled([
        listModelCatalogVendors(),
        listModelCatalogModels(),
        listModelCatalogMappings(),
        getModelConfig(),
      ...
      if (cfg.status === 'fulfilled') setModelConfig(cfg.value)
      else {
        setModelConfig(null)
        toast(cfg.reason instanceof Error && cfg.reason.message ? cfg.reason.message : '加载默认模型配置失败', 'error')
      }
  ...
  const handleSetDefaultModel = React.useCallback(async (slot: ModelConfigDefaultSlot, vendorKey: string, modelKey: string) => {
    setDefaultSavingSlot(slot)
    try {
      await upsertModelConfigDefaultModel(slot, { vendorKey, modelKey })
      toast('Agent 默认模型已更新', 'success')
      await reloadAll()
    } catch (err: unknown) {
      console.error('set default model failed', err)
      toast(err instanceof Error && err.message ? err.message : '默认模型保存失败', 'error')
    } finally {
      setDefaultSavingSlot(null)
    }
  }, [reloadAll])

  const handleClearDefaultModel = React.useCallback(async (slot: ModelConfigDefaultSlot) => {
    setDefaultSavingSlot(slot)
    try {
      await deleteModelConfigDefaultModel(slot)
      toast('Agent 默认模型已清除', 'success')
      await reloadAll()
    } catch (err: unknown) {
      console.error('clear default model failed', err)
      toast(err instanceof Error && err.message ? err.message : '默认模型清除失败', 'error')
    } finally {
      setDefaultSavingSlot(null)
    }
  }, [reloadAll])
  ...
  const handleExport = React.useCallback(async () => {
    ...
    try {
      ...
      const fileName = `canvas-model-catalog-${buildSafeFileTimestamp(now)}.json`
  ...
  const handleExportFull = React.useCallback(async () => {
    ...
    try {
      ...
      const now = new Date()
      const jsonStr = JSON.stringify(pkg, null, 2)
      const fileName = `canvas-model-catalog-full-${buildSafeFileTimestamp(now)}.json`
      downloadTextAsFile(jsonStr, fileName, 'application/json')
  ...
  return (
    <Stack className={rootClassName} gap="md">
      ...
      <StatusOverview
        className="stats-model-catalog-status-overview"
        config={modelConfig}
        busySlot={defaultSavingSlot}
        onJumpToProvider={() => setVendorsModalOpened(true)}
        onClearSlot={(slot) => void handleClearDefaultModel(slot)}
      />
