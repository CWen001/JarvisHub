...
import { useRFStore, computeContextAwarePosition } from '../canvas/store'
...
const REMOTE_RUN_KINDS = new Set(['image', 'video'])
...
export class CanvasService {
  ...
  static async createNode(params: {
    ...
    try {
      ...
      // 位置：仅在调用方显式提供时透传；未提供时交给 store.addNode 内部基于
      // 当前已落库的 nodes 实时计算并经 resolveNonOverlappingPosition 去重。
      // 不要在外层先用 generateDefaultPosition 预计算 —— 多次连续 createNode
      // 会读到同一份 stale snapshot，导致所有节点落到同一锚点（regression）。
  ...
  private static normalizeNodeParams(params: {
    ...
    if (!normalizedKind) {
      throw new Error('当前 taskNode 只保留 text / image / imageEdit / video 四类节点')
    }
    ...
    const mergedConfig: Record<string, any> = { ...baseData, ...restConfig, kind: normalizedKind }
  ...
  private static generateDefaultPosition(): { x: number; y: number } {
    const { nodes } = useRFStore.getState()
    return computeContextAwarePosition(nodes)
  }
