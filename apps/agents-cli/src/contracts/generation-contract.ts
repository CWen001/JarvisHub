...
export function parseGenerationContract(input: unknown): GenerationContractParseResult {
  ...
  const fixedConstraints = normalizeStringList(
    input.fixedConstraints,
  ...
  if (fixedConstraints === null) {
    return {
      ok: false,
      error: "generationContract.fixedConstraints must be an array of strings",
    };
  }
...
export function formatGenerationContractPromptLines(contract: GenerationContract | null): string[] {
  ...
  return [
    ...
    `- fixedConstraints: ${contract.fixedConstraints.length ? contract.fixedConstraints.join(" | ") : "(none)"}`,
