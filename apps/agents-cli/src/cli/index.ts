...
import { loadConfig, loadServeConfig, getAgentsHomeDir, writeGlobalConfig, resolveWorkspaceRoot } from "../core/config.js";
...
import { createCanvasRuntime } from "../runtime/canvas-runtime.js";
  ...
  .action(async (options) => {
    ...
    const server = await startAgentsHttpServer(
      ...
      },
      {
        ...
        maxAllowedTools: runtime.config.maxAllowedTools,
      }
    );
...
function createRuntime(options: { stream?: boolean }): Runtime {
  const cwd = process.cwd();
  const config = loadServeConfig(cwd);
  if (options.stream === false) {
    config.stream = false;
  }
  const runtime = createCanvasRuntime({ cwd, config });
