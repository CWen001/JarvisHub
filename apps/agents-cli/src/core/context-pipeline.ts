...
export async function resolveAgentRunContext(params: {
  ...
  sessionId?: string;
  currentModel: string;
  currentLlmCreds?: { apiKey: string; baseUrl: string; model: string };
  ...
  if (params.currentLlmCreds) {
    runtimeMeta.currentLlmCreds = params.currentLlmCreds;
  }
