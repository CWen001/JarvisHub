...
function isRemoteCanvasTool(toolDefinition: ToolDefinition | undefined): boolean {
  return toolDefinition?.provider?.kind === "remote" && toolDefinition.provider.id === "canvas";
}

export function evaluateToolPolicy(input: PolicyEngineInput): ToolPolicyDecision {
  const permission = input.toolDefinition?.permission;
  const effects = input.toolDefinition?.effects;
  const meta = input.meta;
  const privilegedLocalAccess = meta?.privilegedLocalAccess === true;
  const allowRemoteCanvasSideEffects = isRemoteCanvasTool(input.toolDefinition);
  if (permission?.defaultMode === "deny") {
    return buildDecision(
      "deny",
      `策略拦截：tool ${input.toolName} 默认权限为 deny。`,
      "runtime_grant",
      "tool",
    );
  }
  if (
    !allowRemoteCanvasSideEffects &&
    !privilegedLocalAccess &&
    (permission?.defaultMode === "ask" || effects?.destructive || effects?.costBearing)
  ) {
    return buildDecision(
      "requires_approval",
      `策略拦截：tool ${input.toolName} 声明了需确认的副作用。`,
      "runtime_grant",
      "tool",
    );
  }
