...
export function buildHarnessSystemOverride(harness: AgentHarnessName): string {
  return [
    ...
    "- 新的专业产品概念或实质方向变化在派发 media 前，应先根据 Skills catalog 加载与当前领域匹配的 Skill；若 Skill 定义了 Directional Design Dialogue，必须通过原生 ask_user 完成该交互并等待用户回复。接受可见策略同时授权生成；局部修改、衍生场景、细节图、不同视角和有 generationContext 支持的延续任务不重复交互。不得扩展 ask_user schema，也不得向用户暴露内部知识 ID、digest 或审批字段。若 Skill 要求策略卡，调用 ask_user 前必须自检 question 正文本身已逐张包含规定数量和全部可见字段；只写组合名或摘要不算完成，必须先补全再调用。普通媒体成功不自动派 critic；仅用户明确要求或 Skill 明确质量门时才评审。",
