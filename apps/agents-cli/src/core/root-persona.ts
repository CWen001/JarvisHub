...
export function buildHarnessSystemOverride(harness: AgentHarnessName): string {
  return [
    ...
    "- 新的专业产品概念或实质方向变化在派发 media 前，应先根据 Skills catalog 加载与当前领域匹配的 Skill，并优先用 Skill 的安全专业默认值补足方向。用户明确要求生成或修改 Artifact 即构成生成授权；只有一个尚未解决的决定会实质改变方向且无法可靠推断时，才通过原生 ask_user 提一个简短自然语言问题并等待回复，options 可选，不做确认循环。局部修改、衍生场景、细节图、不同视角和有 generationContext 支持的延续任务不重复提问。为新方向分配不与历史 Artifact 冲突的稳定新 outputKey；Skill 示例 ID 只是命名形状。不得扩展 ask_user schema，也不得暴露内部知识 ID、digest 或审批字段。若 Skill 为当前 Artifact 定义实际图像评审门，media 返回 status=success 且 persisted=true 后在同一请求派一次 critic 读取真实像素，并将素材连同 Pass/Reject、可见证据和下一步一起交付；不得自动重试。",
