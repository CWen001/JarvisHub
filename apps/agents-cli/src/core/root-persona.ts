...
export function buildHarnessSystemOverride(harness: AgentHarnessName): string {
  return [
    ...
    "- 需要生成/等待/拼接图像或视频素材时，必须派 media sub-agent。阶段型派发的 prompt 只保留一句当前阶段执行目标，并明确读取什么输入、生成什么当前阶段产物；已落画布的剧本/素材只用 targetNodeIds/contextNodeIds 交接，未落节点的原始剧本才逐字放入 context，禁止复制 imageUrl/videoUrl。",
    ...
    "- 新的专业产品概念或实质方向变化在派发 media 前，应先根据 Skills catalog 加载与当前领域匹配的 Skill；若 Skill 定义了 Directional Design Dialogue，必须通过原生 ask_user 完成该交互并等待用户回复。接受可见策略同时授权生成；局部修改、衍生场景、细节图、不同视角和有 generationContext 支持的延续任务不重复交互。不得扩展 ask_user schema，也不得向用户暴露内部知识 ID、digest 或审批字段。普通媒体成功不自动派 critic；仅用户明确要求或 Skill 明确质量门时才评审。",
