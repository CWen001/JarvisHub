...
import { syncToRemoteMemory } from "./memory-sync.js";
...
export function createMemoryTools(defaultMemoryRoot: string): ToolHandler[] {
  ...
  const saveTool: ToolHandler = {
    definition: {
      ...
      parameters: {
        type: "object",
        properties: {
          title: { type: "string", description: "简短标题（3-8个词），用于索引和展示" },
          ...
          scopeType: {
            type: "string",
            enum: ["user", "project"],
            description: "记忆作用域。默认随当前画布上下文（有 projectId 写 project，否则写 user）；显式 'user' 写入全局，'project' 强制写当前画布。",
          },
        },
    ...
    async execute(args, ctx, toolCallId) {
      ...
      const scopeType = parseScopeType(args.scopeType);
  ...
  const forgetTool: ToolHandler = {
    ...
    async execute(args, ctx, toolCallId) {
      ...
      syncToRemoteMemory(ctx.meta, { action: "forget", ids: result.ids });
      syncArtifacts(ctx.meta);
  ...
  const allTools = [saveTool, searchTool, forgetTool, reflectTool, reflectCommitTool];
  return allTools.map((tool): ToolHandler => ({
    ...tool,
    async execute(args: Record<string, unknown>, ctx: ToolContext, toolCallId: string) {
      if (ctx.meta?.disableMemory === true) {
        return { toolCallId, content: JSON.stringify({ skipped: true, reason: "memory disabled" }) };
      }
      return tool.execute(args, ctx, toolCallId);
    },
  }));
}
...
function parseScopeType(input: unknown): "user" | "project" | undefined {
  if (typeof input !== "string") return undefined;
  const value = input.trim().toLowerCase();
