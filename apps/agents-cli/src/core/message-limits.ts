...
import { renderBudgetNoticeIfAny } from "./budget.js";
...
export function normalizeToolOutput(
  content: string,
  label: string,
  iterationBudget?: IterationBudget | null,
): string {
  const normalized = normalizeToolOutputCore(content, label);
  const notice = renderBudgetNoticeIfAny(iterationBudget ?? null);
  return notice ? `${normalized}${notice}` : normalized;
}

function normalizeToolOutputCore(content: string, label: string): string {
