...
export function buildMemoryPromptFragment(input: MemoryPromptFragmentInput): string {
  ...
  const includeCrossSessionMemory = true;
  const lines = [
    "## Persisted Memory",
    "当前 session recall 和跨 session 记忆均已启用。参考已持久化的创作偏好、角色设定与决策来保持项目一致性。",
  ];
  ...
  if (includeCrossSessionMemory) {
    ...
    if (prompt) {
      ...
      const relevantItems = searchLayeredMemory({
        ...
        if (item.kind === "session_rollup" && item.sessionId === activeSession?.sessionId) return false;
        const content = String(item.content || "").trim().toLowerCase();
        if (content.length < 10) return false;
        if (/^(hi|hello|你好|hey)\s*$/.test(content)) return false;
        if (content.includes("runtime_completion_self_check")) return false;
        return true;
      });
      const recentSessions = listSessionRollups(memoryRoot, input.maxRecentSessions ?? 3)
        .filter((item) => {
          if (item.sessionId === activeSession?.sessionId) return false;
          const prompt = String(item.latestUserPrompt || "").trim().toLowerCase();
          if (/^(hi|hello|你好|hey)\s*$/.test(prompt)) return false;
          return true;
        });
