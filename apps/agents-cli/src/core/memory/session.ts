...
function readSessionMessagesFromFile(filePath: string, maxMessages: number): Message[] {
  ...
  try {
    ...
    throw new Error(
      `Failed to read session history file: ${filePath}: ${readErrorMessage(err)}`,
    );
  }
}
...
function readSessionIndex(dir: string): SessionIndexRecord[] {
  ...
  try {
    ...
    if (!Array.isArray(parsed)) {
      throw new Error("session index root must be an array");
    }
    ...
    throw new Error(
      `Failed to read session index file: ${filePath}: ${readErrorMessage(err)}`,
    );
  }
}
