---
name: canvas-design-to-web
description: 基于用户提供的设计稿图片还原真实网页。该 skill 可用于任意前端项目，但强依赖本地已启动且已授权的 JarvisHub vision 工作流；视觉模型由服务端默认配置决定，支持外部传入 vision prompt，且不可静默降级。
---

# JarvisHub Design To Web
...
- 在 JarvisHub `webHero` 画布流程中，只生成 HTML/CSS，并通过 readiness -> 两字段 stage -> commit 提交；`webHeroDocumentHtml` 必须由服务端派生，不能写进 text node 或 generic node patch。

## 强约束

- 本 skill 强依赖本地已启动的 JarvisHub vision 工作流。
- 视觉模型由服务端默认配置决定，调用方不要在工具参数里传任何未暴露的模型 / 厂商选择字段。
- 假定当前环境具备白名单调用权限；若无权限，必须直接报错。
- 必须先做视觉取证，再动代码；禁止凭主观印象直接开写。
- 若代码生成依赖画布里的已批准预览图，必须先用 `canvas_read_node_media_for_context` 读取这些预览图，让模型真实看到图片后再写最终代码。
- 每张已批准预览图都必须对应最终 DOM 中一个真实 `<section>`；CSS/JS 引用的 section id 必须存在于 DOM 中。
...
- 明确还原标准：
  - 高保真还原
  - 保留风格但允许工程化调整
- 确认本次任务应通过本地 JarvisHub vision 工作流做视觉分析。
...
优先使用本地 JarvisHub vision 工作流，对设计稿做结构化取证。目标不是“看图讲故事”，而是提取可编码的界面事实。
...
- 视觉模型由服务端默认配置决定；**不要**在工具参数里传任何未暴露的模型 / 厂商选择字段，传入会被拒。
- 若当前 schema 支持温度参数，保持低值。
...
- 页面拆成哪些组件
- JarvisHub `webHero` 目标节点 id，以及最终要写入的 `webHero*` 字段
...
- 优先改真实页面，不要只生成示例文件。
- 若目标是 JarvisHub `webHero`，用 `canvas_create_webhero_node` 确保存在目标节点；先通过 `canvas_webhero_check_readiness`，再用稳定 session ID 只 stage HTML/CSS，最后调用 `canvas_webhero_code_commit`。若宿主是严格 JSON 的前端 WebHero runner，只把 HTML/CSS JSON 返回给宿主，由 runner 执行同一事务；任何路径都不得用 `canvas_update_node_data` 写最终代码。
