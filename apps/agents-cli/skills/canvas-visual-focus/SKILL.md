---
name: canvas-visual-focus
description: 基于图片理解提取“视觉重点/注意力分配/关键要素”，并根据用户对话目标输出可执行的要点清单与提示词骨架（默认用 canvas_vision；模型由服务端默认配置决定）。（素材生成场景：从单张参考图提取视觉要点、生成图片 prompt 骨架。）
---

# JarvisHub 视觉重点提取（Visual Focus）

适用场景：
- 用户给了图片（URL 或 DataURL），并希望你提取“视觉重点/关键要素/注意力在哪里/风格要点/拍摄布光要点”。
- 用户的目标来自对话：例如“复现这张图”“做同风格海报”“做镜头图片/视频风格对齐”“提炼这张图的视觉主轴”。
...
3) **调用图片理解工具（必做）**
- 使用 `canvas_vision`。
- 若当前 schema 支持温度参数，默认保持低值。
- 视觉模型由服务端默认配置决定，不要在工具参数里传任何未暴露的模型 / 厂商选择字段。
- 若上游已经提供外部 `prompt`，直接透传；若未提供，再使用下方推荐模板。

建议给视觉任务 prompt 的模板（把对话目标填进去）：
...
- 调用参数以当前 `canvas_vision` schema 为准：必须提供真实图片输入和任务 prompt；若 schema 支持温度参数，使用低值；不要传未暴露的模型 / 厂商选择字段。
