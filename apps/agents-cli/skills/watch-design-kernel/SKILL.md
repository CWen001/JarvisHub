...
3. Present one Recommended Strategy Composition through the unchanged `ask_user` schema. The initial options must be exactly `按此策略生成` and `调整策略`; do not dispatch Media in the same turn because `ask_user` stops the turn.
