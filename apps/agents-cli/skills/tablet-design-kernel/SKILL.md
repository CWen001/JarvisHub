...
2. Before every Tablet image-generation request, run one native **Design Direction Turn** unless the user explicitly waives it by asking to use the recommendation or proceed without discussion. One turn covers one user request, including a same-direction batch; never ask once per Provider call.
