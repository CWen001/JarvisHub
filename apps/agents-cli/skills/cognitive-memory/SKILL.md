...
- `memory_save({ title, content, tags?, store?, source?, importance? })`
...
1. `title` 为简短标题（3-8个词），用于在 Memory Panel 中展示索引，例如"偏好深色主题"、"项目使用 pnpm"。
2. `content` 为完整记忆内容，包含具体细节和上下文。
3. `tags` 至少包含任务域和对象（例如 `project-metadata`, `style:xxx`）。
4. `source` 记录来源技能或流程（例如 `agents-team-metadata`）。
5. `importance` 按 0~1 给出，核心约束建议 >= 0.8。
